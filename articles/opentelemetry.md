...
‘shinyOAuth’ includes OpenTelemetry (OTel) instrumentation emitting logs
and traces from key OAuth 2.0/OIDC operations.

The `otel` package is installed automatically with `shinyOAuth`. Install
`otelsdk` as well if you want the R SDK helpers/exporters used in the
examples below.

OpenTelemetry is an open standard for telemetry data (logs, traces,
metrics) and can be collected by various backends. It can be used as an
alternative to the package’s native R hooks for auditing and tracing
(see
...
to learn more about configuring your OpenTelemetry setup and exporters
in R. Once configured, ‘shinyOAuth’ will automatically emit OTel
signals.
...
- Successful operations are marked with status `ok`; errors are marked
  ...
  message
- Top-level shinyOAuth operation spans are started with explicit root
  parenting where appropriate so they are not hidden under Shiny’s
  internal `reactive_update` spans
- When a callback can be matched back to the originating login request,
  `shinyOAuth.callback` is parented to that original
  `shinyOAuth.login.request` span even when the redirect returns in a
  different Shiny session; otherwise it falls back to a root span
- HTTP client spans record both `http.response.status_code` and
  `http.response.content_type` on the `*.http` span itself once a
  response is available; non-success HTTP responses mark that client
  span as `error`
...
For shinyOAuth-managed async work, the package captures the parent
session’s relevant `OTEL_*` / `OTEL_R_*` environment variables and
reapplies them inside the async execution context before running OAuth
logic. This keeps exporter selection and OTLP endpoints aligned across
the main R process and async evaluation backends such as mirai workers
and future-based execution. For long-lived workers that are reused
across tasks this is best effort, because reconfiguring
