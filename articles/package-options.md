...
This reference lists package-wide options for network requests, logging,
and validation. Set options near the top of `app.R`, before creating the
provider and client. The defaults apply when an option is unset.
...
Options affect the R process; use
[`oauth_client()`](https://lukakoning.github.io/shinyOAuth/reference/oauth_client.md)
and
[`oauth_module_server()`](https://lukakoning.github.io/shinyOAuth/reference/oauth_module_server.md)
arguments for settings that belong to one client or module. See
...
for details about audit hooks, and the [OpenTelemetry
...
from being passed via `extra_auth_params`, `extra_token_params`, and
`extra_token_headers`. This helps prevent accidental misconfiguration
that could break state binding, PKCE, or client authentication.
...
These limits reject oversized callback URLs and POST bodies before they
