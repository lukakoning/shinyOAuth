...
exchange and refresh, shinyOAuth does not automatically replay a
`DPoP-Nonce` challenge because those requests may consume a single-use
credential. Instead it raises a typed error so the caller can decide
whether replay is safe. The response must include at least
...
- Authorization-code exchange and refresh calls to the token endpoint
  ...
  `DPoP-Nonce` challenge, shinyOAuth surfaces a typed nonce-challenge
  error instead of replaying the same non-idempotent request
  automatically
- Later
