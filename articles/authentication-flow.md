...
Configured authorization endpoint queries are preserved. Matching
managed singleton values are emitted once; conflicting or repeated
singleton values are rejected before use. Repeated `resource` indicators
remain supported, and parameter names are case sensitive. The same
