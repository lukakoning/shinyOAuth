...
emits a structured audit or error event. The function you register
should be fast, non-blocking, and never throw errors.
...
Another way that ‘shinyOAuth’ can emit audit events is via OpenTelemetry
logging. Please see
...
- `trace_id`: a short correlation id for linking related records from
  the same operation or auth flow
...
exchange, and login success or failure handling. The trace id is
embedded inside the encrypted state payload so it survives the browser
...
log attribute, which is distinct from the backend’s native OpenTelemetry
trace/span ids.
...
- `shiny_session$is_async`: a logical indicating whether the event was
  ...
  reactive flow.
- If you pre-capture a worker-intended context with
  `capture_shiny_session_context(is_async = TRUE)` but emit the event
  before any worker dispatch actually happens, shinyOAuth normalizes
  that borrowed context back to `FALSE` because the event was still
  emitted on the main process.
...
incrementing a counter inside `audit_hook` only affect the worker’s
private copy. For cross-process delivery, write to an external sink
(database, file, queue, OTLP exporter, etc.) or explicitly return
...
`options(shinyOAuth.audit_digest_key = ...)`, each R process gets its
own random key at runtime. This reduces the risk of correlation or
dictionary reidentification if logs leak, but it also means digests are
not stable across unrelated processes by default.
...
Note: unkeyed digests are pseudonymous, so not anonymized—low-entropy
...
- When: a package error is raised via one of the specialized `err_*()`
  helpers (state validation, PKCE, token, ID token, userinfo,
  configuration, input, parse errors)
- Fields:
