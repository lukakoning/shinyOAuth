...
‘shinyOAuth’ emits structured audit events as the OAuth 2.0/OIDC flow
runs. These events can help you understand what happened during login
and spot problems such as repeated failures, replay attempts, or
...
record back to the app session and HTTP request. The structure is
...
These let you connect related events without logging the raw sensitive
values themselves.
...
own random key at runtime. That makes accidental cross-process matching
...
Note: unkeyed digests are easier to compare across systems, but they are
also easier to guess for low-entropy values such as email addresses.
