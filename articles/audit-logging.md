...
- `is_async` and `process_id`: whether work ran in a background worker
  ...
  `main_process_id`.
- `http`: a summary with request method, host, and scheme by default.
...
By default, the HTTP summary omits paths, query strings, headers, and
client addresses. The raw Shiny session token is also omitted. To omit
...
configure `shinyOAuth.telemetry_path_scrubber` as a function of the path
returning a safe absolute path (for example `/users/:id`), or `NULL` to
omit it. Use an allowlist or replace all identifying path segments; the
function should be idempotent. Scrubber errors and invalid results omit
the path. Scrubber input is limited to 2048 bytes; exported paths to 512
bytes. Method, host, and scheme are limited to 32, 255, and 16 bytes.
...
- When: a multi-provider registry rejects a callback before selecting a
  client.
- Context: `phase` (`callback_registry_routing`), `reason`, `status`
  (`error`), and sanitized HTTP metadata in `shiny_session.http`.
- Reasons: `route_unavailable`, `route_unregistered`,
  `unexpected_transport`, `issuer_missing`, and `issuer_unrecognized`.
- The `shinyOAuth.callback.route` span records the same phase and reason
  with error status. No provider identity is assigned from unvalidated
  issuer input.
- Parser errors and later client-specific validation retain their
  existing diagnostics; they do not also emit this routing event.
...
- Fields:
  ...
  - `url`: the request scheme and authority, plus an approved path only
    when a path scrubber is configured
