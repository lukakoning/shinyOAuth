...
* Pull-request pkgdown builds now run with read-only repository permissions and
without persisted checkout credentials. GitHub Pages deployment runs in a
separate non-PR workflow that alone receives `contents: write` permission.
...
requires a complete set of canonically distinct redirect routes and is limited
to module callback handling, where the received route can be verified.
...
still applies an explicit endpoint allowlist, and JWKS keeps its separate
issuer-host pinning policy by default.
