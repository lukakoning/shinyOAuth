...
for (index in seq_len(nrow(cases))) {
  ...
  testthat::test_that(
    paste(
      "same issuer, shared route and isolated resources",
      response_mode,
      async
    ),
    {
      ...
      retention_browser_wait(
        b,
        function() retention_browser_snapshot(b),
        "second tab"
      )
      ...
      finish <- function(browser, count) {
        ...
        testthat::expect_identical(
          retention_browser_value(browser, "location.pathname"),
          "/callback/shared"
        )
        testthat::expect_identical(
      ...
      first <- finish(a, 1L)
      testthat::expect_identical(
        first[["connections"]][[1L]][["client_label"]],
        "Site a"
      )
      both <- finish(b, 2L)
      ...
      metrics <- function() {
        httr2::request(f[["providers"]][["shared"]][["url"]]("/metrics")) |>
          httr2::req_timeout(5) |>
          httr2::req_perform() |>
