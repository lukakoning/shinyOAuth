...
for (async in c(FALSE, TRUE)) {
  for (response_mode in c("query", "form_post")) {
    testthat::test_that(
      paste("Strict owner callbacks", response_mode, "async =", async),
      {
        f <- retention_browser_setup(
          async,
          response_mode,
          provider_factory = function(site, callback) {
            retention_fixture_provider(
              site,
              callback,
        ...
        browser[["Network"]][["responseReceived"]](function(params) {
          ...
          if (
            identical(params[["type"]], "Document") &&
              startsWith(response[["url"]], paste0(f[["origin"]], "/callback/"))
          ) {
            callback_responses[[length(callback_responses) + 1L]] <<- list(
              status = response[["status"]],
              sets_cookie = "set-cookie" %in%
                tolower(names(response[["headers"]]))
            )
          }
        })
        owner_cookie <- function() {
          Filter(
            function(cookie) isTRUE(cookie[["httpOnly"]]),
            browser[["Network"]][["getCookies"]](urls = list(f[["origin"]]))[[
              "cookies"
            ]]
          )
        }
        ...
        completed <- retention_browser_wait(
          browser,
          function() {
            snapshot <- retention_browser_snapshot(browser)
            if (!is.null(snapshot) && length(snapshot[["connections"]]) == 1L) {
              return(snapshot)
            }
            if (
              isTRUE(retention_browser_value(
                browser,
                "document.body.textContent.includes('The local owner session ended')"
              ))
            ) {
              return("owner ended")
            }
        ...
        if (is.list(completed)) {
          ...
          authorize()
          browser[["Network"]][["deleteCookies"]](
            name = initial_cookie[["name"]],
            url = f[["origin"]]
          )
          ...
          testthat::expect_equal(
            vapply(callback_responses, `[[`, 0, "status"),
            c(200, 200, 200, 400)
          )
          ...
          metrics <- httr2::request(f[["providers"]][["a"]][["url"]](
            "/metrics"
          )) |>
            httr2::req_perform() |>
