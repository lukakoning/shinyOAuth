...
for (index in seq_len(nrow(cases))) {
  ...
  testthat::test_that(
    paste(
      "repeated same-client authorization",
      case[["response_mode"]],
      "async",
      case[["async"]],
      "reused credentials",
    ...
    ),
    {
      ...
      authorize <- function(count) {
        ...
        snapshot <- retention_browser_wait(
          browser,
          function() {
            snapshot <- retention_browser_snapshot(browser)
            if (
              !is.null(snapshot) && length(snapshot[["connections"]]) == count
            ) {
              snapshot
            } else {
              NULL
      ...
      select <- function(id) {
        retention_browser_value(
          browser,
          paste0(
            "Shiny.setInputValue('selected_id',",
      ...
      testthat::expect_true(all(vapply(
        both[["connections"]],
        function(row) identical(row[["client_label"]], "Site a"),
        logical(1)
      )))
      ...
      restored <- retention_browser_wait(
        ...
        function() {
          ...
          if (
            !is.null(snapshot) &&
              snapshot[["session"]] > both[["session"]] &&
              length(snapshot[["connections"]]) == 2L
          ) {
            snapshot
          } else {
            NULL
