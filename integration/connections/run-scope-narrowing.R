...
run_scope_narrowing_browser_tests <- function() {
  ...
  artifacts <- file.path(
    ...
    paste0("scopes-", format(Sys.time(), "%Y%m%d-%H%M%S"))
  )
  ...
  results <- testthat::test_file(
    "integration/connections/test-browser-scope-narrowing.R",
    env = environment(),
    reporter = "summary",
  ...
  if (evidence[["failures"]] || evidence[["errors"]] || evidence[["skipped"]]) {
    stop("Refresh scope browser gate did not fully pass")
  }
  evidence[["status"]] <- "passed"
  cat(
    "Refresh scope browser gate:",
    evidence[["passed"]],
