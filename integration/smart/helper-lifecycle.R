lifecycle_setup <- function(
  ...
  factory <- function(site, callback) {
    smart_profile_provider(
      ...
      "standalone",
      https = TRUE,
      behavior = if (site == "a") behavior else list()
    )
  }
  ...
  f[["control"]] <- function(path, site = "a") {
    httr2::request(paste0(f[["bases"]][[site]], "/test/", path)) |>
      httr2::req_options(
        cainfo = file.path(
          retention_root,
          "integration/keycloak/tls/ca-cert.pem"
