...
smart_profile_app <- function(
  ...
  options(shinyOAuth.timeout = http_timeout)
  if (async) {
    mirai::daemons(2L)
  ...
  clients <- lapply(c("a", "b"), function(site) {
    discovery <- shinyOAuth::smart_discover(
      paste0(providers[[site]], "/fhir"),
  ...
  base_ui <- shiny::fluidPage(
    ...
    shiny::actionButton("disconnect_a", "Disconnect A"),
  ...
  server <- function(input, output, session) {
    ...
    perform <- function(fn) {
      ...
      if (inherits(value, "promise")) {
        ...
        complete(if (isTRUE(value)) "refreshed" else value)
      }
    }
    for (site in c("a", "b")) {
      local({
        ...
        shiny::observeEvent(
          ...
          perform(function() connection(selected)[["refresh"]]())
        )
        shiny::observeEvent(
          ...
          perform(function() {
            ...
            stopifnot(
              identical(body[["id"]], ctx[["patient"]]),
              !identical(
                ctx[["patient"]],
                sub("^Practitioner/", "", ctx[["fhirUser"]])
              )
            )
            paste0(
              body[["fixture_site"]],
              ":",
              body[["fixture_revision"]],
              ":context-",
              ctx[["revision"]]
            )
          })
        ...
        shiny::observeEvent(
          ...
          perform(function() {
            ...
            stopifnot(
              identical(body[["resourceType"]], "Practitioner"),
              identical(body[["id"]], paste0("clinician-", selected))
            )
        ...
        shiny::observeEvent(
          ...
          perform(function() {
            ...
            stopifnot(identical(
              httr2::resp_body_json(response)[["resourceType"]],
              "Bundle"
            ))
    ...
    shiny::observeEvent(
      input[["narrow_a"]],
      perform(function() connection("a")[["refresh"]](scopes = limited))
    )
    shiny::observeEvent(
      input[["widen_a"]],
      perform(function() {
    ...
    shiny::observeEvent(
      input[["disconnect_b"]],
      health[["disconnect"]](
        connection("b")[["summary"]]()[["connection_id"]],
        revoke = FALSE
      )
    )
    shiny::observeEvent(
      input[["disconnect_a"]],
      health[["disconnect"]](
        connection("a")[["summary"]]()[["connection_id"]],
        revoke = FALSE
      )
    )
    ...
    shiny::observeEvent(
      ...
      perform(function() {
        ...
        stopifnot(
          !is.null(expected_context),
          identical(ctx[["patient"]], expected_context[["patient"]]),
          identical(ctx[["encounter"]], expected_context[["encounter"]]),
          identical(ctx[["revision"]], expected_context[["revision"]]),
  ...
  routes <- if (launch == "ehr") {
    list(shinyOAuth::smart_launch_route("/launch", c("a", "b")))
  } else {
    list()
  }
  ...
  shiny::runApp(
    ...
    host = "127.0.0.1",
    port = if (is.null(listen_port)) {
      as.integer(httr2::url_parse(origin)[["port"]])
    } else {
      listen_port
    },
