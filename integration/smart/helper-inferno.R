...
inferno_wait <- function(predicate, description, timeout = 60) {
  ...
  repeat {
    ...
    if (Sys.time() >= deadline) {
      stop("Timed out: ", description, call. = FALSE)
    }
    Sys.sleep(0.1)
  }
}
...
inferno_docker <- function(args, log = NULL, timeout = 120000) {
  ...
  if (result[["status"]] != 0L) {
    stop(
      "Inferno Docker operation failed (",
      result[["status"]],
      "); inspect the local private log: ",
      log,
      call. = FALSE
    )
  }
...
inferno_build <- function(root, output) {
  inferno_docker(
    c(
      ...
      "--tag",
      inferno_image,
      file.path(root, "integration/smart/inferno")
    ),
  ...
  changed <- trimws(inferno_docker(c(
    "run",
    "--rm",
    "--entrypoint",
    "git",
  ...
  revision <- trimws(inferno_docker(c(
    "run",
    "--rm",
    "--entrypoint",
    "git",
  ...
  list(
    ...
    kit_version = "1.0.3",
    core_version = "1.4.3",
    image_id = trimws(inferno_docker(c(
      "image",
      "inspect",
      "--format",
      "{{.Id}}",
    ...
    upstream_verification_unchanged = TRUE,
    patch_sha256 = paste(openssl::sha256(readBin(
      patch,
      "raw",
      n = file.size(patch)
    )))
...
inferno_tls <- function(root, target_port, .env = parent.frame()) {
  ...
  if (!nzchar(python)) {
    stop("Inferno browser integration requires Python 3")
  }
  proxy <- processx::process[["new"]](
    python,
    c(
      file.path(root, "integration/connections/tls-proxy.py"),
      "--target-port",
      as.character(target_port),
      "--cert",
      file.path(root, "integration/keycloak/tls/server-cert.pem"),
      "--key",
      file.path(root, "integration/keycloak/tls/server-key.pem")
    ),
    stdout = "|",
    stderr = "|",
    supervise = TRUE
...
inferno_test_simulator <- function(root, output) {
  inferno_docker(
    c(
      ...
      paste0(
        "type=bind,source=",
        file.path(root, "integration/smart/inferno/simulator_spec.rb"),
      ...
      "--format",
      "progress",
      "spec/shinyoauth_simulator_spec.rb",
      "spec/smart_app_launch/client_suite/mock_smart_server_spec.rb"
    ),
...
inferno_api <- function(stack, path, data = NULL, json = TRUE) {
  req <- httr2::request(paste0(stack[["origin"]], path)) |>
    httr2::req_options(cainfo = stack[["ca"]]) |>
...
inferno_start <- function(root, output, site = "a", .env = parent.frame()) {
  ...
  writeLines(
    c(
      paste0("SMART_INFERNO_ORIGIN=", tls[["origin"]]),
      paste0("SMART_INFERNO_PORT=", port)
    ),
    env_file
  )
  compose <- c(
    ...
    env_file,
    "--file",
    file.path(root, "integration/smart/inferno/compose.yml")
  )
  ...
  withr::defer(
    inferno_docker(
      ...
      file.path(output, paste0(site, "-cleanup.log"))
    ),
    envir = .env
  )
  inferno_docker(
    c(compose, "up", "--detach", "redis"),
    file.path(output, paste0(site, "-redis.log"))
  )
  inferno_docker(
    c(
      compose,
    ...
    file.path(output, paste0(site, "-migrate.log"))
  )
  inferno_docker(
    c(compose, "up", "--detach", "--no-build", "inferno", "worker"),
