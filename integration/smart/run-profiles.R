...
run_smart_profiles <- function(args = commandArgs(trailingOnly = TRUE)) {
  if (!all(args %in% c("--quick", "--post", "--es384", "--cross-site"))) stop("Usage: Rscript integration/smart/run-profiles.R [--quick] [--post] [--es384] [--cross-site]")
  ...
  evidence <- list(gate = "SMART profile browser matrix", status = "failed", external_conformance = FALSE,
    complete_matrix = !any(c("--quick", "--es384", "--cross-site") %in% args), scenarios = cases, server = "strict synthetic SMART fixture",
    authorization_method = authorization_method, extra_scope_count = length(extra_scopes),
    transport = if (cross_site) "HTTPS cross-site: 127.0.0.1 app / localhost providers" else "HTTP loopback development exception",
