run_permissions <- function() {
  ...
  output <- file.path(
    ...
    paste0("permissions-", format(Sys.time(), "%Y%m%d-%H%M%S"))
  )
  dir.create(output, recursive = TRUE)
  evidence <- list(
    status = "failed",
    server = "Microsoft FHIR Server 5.0.58",
    server_modified = FALSE,
    authorization_server = "synthetic SMART fixture",
    unmodified_external_smart_interoperability = "not_established",
    scenarios = list()
  )
  on.exit(
    jsonlite::write_json(
  ...
  cat(
    "Protected FHIR: two browser scenarios passed against Microsoft FHIR Server.\n"
  )
}
run_permissions()
