...
smart_mixed_app <- function(
  ...
  provider <- shinyOAuth::oauth_provider(
    name = "OIDC",
    issuer = paste0(providers[["b"]], "/fhir"),
    auth_url = paste0(providers[["b"]], "/authorize"),
    token_url = paste0(providers[["b"]], "/token"),
    jwks_uri = paste0(providers[["b"]], "/keys"),
    userinfo_url = paste0(providers[["b"]], "/userinfo"),
  ...
  ordinary <- shinyOAuth::oauth_client(
    provider,
    "b",
    redirect_uri = callbacks[[2]],
    scopes = c("openid", "profile", "offline_access"),
    required_scopes = c("openid", "profile"),
    resource_bases = c(api = paste0(providers[["b"]], "/api")),
    label = "OIDC",
    response_mode = response_mode,
    authorization_server_mode = "multi_redirect_uri",
    authorization_server_redirect_uris = callbacks
  ...
  clients <- if (managed_oidc) {
    list(a = smart, b = ordinary)
  } else {
    list(a = smart)
  }
  ...
  server <- function(input, output, session) {
    ...
    legacy <- if (!managed_oidc) {
      shinyOAuth::oauth_connection(ordinary, shiny::reactive(login[["token"]]))
    } else {
      NULL
    }
    connection <- function(label) {
      if (label == "OIDC" && !managed_oidc) {
        return(legacy)
      }
      rows <- Filter(
        function(row) identical(row[["client_label"]], label),
        health[["connections"]]()
      )
      if (!length(rows)) {
        stop("Unavailable")
    ...
    shiny::observeEvent(
      input[["connect_b"]],
      if (managed_oidc) health[["connect"]]("b") else login[["request_login"]]()
    )
    shiny::observeEvent(
      input[["read_a"]],
      perform(function() {
    ...
    shiny::observeEvent(
      ...
      perform(function() {
        ...
        stopifnot(
          identical(value[["id_token_claims"]][["sub"]], "clinician-b"),
          identical(value[["userinfo"]][["name"]], "Synthetic OIDC Name")
        )
        if (!managed_oidc) {
          stopifnot(
            !length(login[["token"]]@smart_context),
    ...
    shiny::observeEvent(
      input[["refresh_a"]],
      perform(function() connection("SMART")[["refresh"]]())
    )
    if (managed_oidc) {
      shiny::observeEvent(
        input[["refresh_b"]],
    ...
    shiny::observeEvent(
      input[["disconnect_a"]],
      health[["disconnect"]](connection("SMART")[["id"]], revoke = FALSE)
    )
    shiny::observeEvent(
      input[["logout_b"]],
      if (managed_oidc) {
        health[["disconnect"]](connection("OIDC")[["id"]], revoke = FALSE)
      } else {
        login[["logout"]]()
      }
    )
    ...
    output[["snapshot"]] <- shiny::renderText(jsonlite::toJSON(
      list(
        ...
        oidc_authenticated = tryCatch(
          connection("OIDC")[["is_usable"]](),
          error = function(...) FALSE
        ),
        oidc_expiry = tryCatch(
          connection("OIDC")[["summary"]]()[["expires_at"]],
          error = function(...) NULL
        ),
        result = result(),
