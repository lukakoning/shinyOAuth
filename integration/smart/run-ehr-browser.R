...
run_smart_ehr_browser_tests <- function() {
  ...
  output <- file.path(
    ...
    paste0("ehr-", format(Sys.time(), "%Y%m%d-%H%M%S"))
  )
  dir.create(output, recursive = TRUE)
  evidence <- list(
    gate = if (quick) "P5a sync/query diagnostic" else "P5a",
    status = "failed",
  ...
  results <- testthat::test_file(
    "integration/smart/test-browser-ehr-launch.R",
    env = environment(),
    reporter = "summary",
  ...
  if (evidence[["failures"]] || evidence[["errors"]] || evidence[["skipped"]]) {
    stop("Incomplete EHR browser gate")
  }
  evidence[["status"]] <- "passed"
  cat(
    "EHR browser assertions:",
    evidence[["passed"]],
    "; sanitized evidence:",
    output,
    "\n"
