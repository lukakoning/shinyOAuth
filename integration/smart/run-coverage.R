...
run_smart_coverage <- function(args = commandArgs(trailingOnly = TRUE)) {
  if (!all(args %in% c("--integration-only", "--require-external"))) {
    stop(
      "Usage: Rscript integration/smart/run-coverage.R [--integration-only] [--require-external]"
    )
  }
  ...
  output <- file.path(
    ...
    paste0("coverage-", format(Sys.time(), "%Y%m%d-%H%M%S"))
  )
  dir.create(output, recursive = TRUE)
  evidence <- list(
    status = "started",
    external_interoperability = "not_established",
    independent_client_verification = "not_run",
    package_version = as.character(utils::packageVersion("shinyOAuth")),
    unit_suite_requested = !"--integration-only" %in% args,
    suites = list()
  )
  on.exit(
    jsonlite::write_json(
  ...
  suites <- list(
    ...
    oauth_authorization_post = c(
      "integration/connections/run-tests.R",
      "--post"
    ),
    ...
    inferno_identity = c(
      "integration/smart/run-inferno-extensions.R",
      "identity"
    ),
    inferno_account = c(
      "integration/smart/run-inferno-extensions.R",
  ...
  if (!"--integration-only" %in% args) {
    suites <- c(
      list(unit_and_browser = c("integration/smart/run-package-tests.R")),
      suites
    )
  }
  ...
  for (name in names(suites)) {
    ...
    if (name == "inferno_client") {
      proof <- tryCatch(
        jsonlite::read_json(report, simplifyVector = FALSE),
        error = function(...) NULL
      )
      if (result[["status"]] == 0L && inferno_report_passed(proof)) {
        evidence[[
          "independent_client_verification"
        ]] <- "passed_modified_inferno_simulator"
        evidence[["suites"]][[name]][["scenarios"]] <- length(proof[[
          "scenarios"
        ]])
        evidence[["suites"]][[name]][["upstream_tests_passed"]] <- 10L *
          length(proof[["scenarios"]])
        evidence[["suites"]][[name]][[
          "evidence_file"
        ]] <- "inferno-evidence.json"
      } else {
  ...
  if ("--require-external" %in% args) {
    evidence[["suites"]][["unmodified_external_app_interoperability"]] <- list(
      status = "unverified",
      exit_status = 1L,
  ...
  failed <- names(Filter(
    function(row) row[["exit_status"]] != 0L,
    evidence[["suites"]]
  ))
  evidence[["status"]] <- if (length(failed)) {
    "failed"
  } else {
    "implemented_suites_passed"
  }
  message("Coverage evidence: ", file.path(output, "evidence.json"))
  if (length(failed)) {
    stop("Incomplete roadmap coverage: ", paste(failed, collapse = ", "))
  }
  # A diagnostic sandbox pass cannot establish application interoperability.
  message(
    "Implemented suites passed, including independent Inferno client verification with recorded simulator corrections."
  )
  message(
    "Unmodified external SMART interoperability remains unestablished; see coverage.md."
  )
}
run_smart_coverage()
