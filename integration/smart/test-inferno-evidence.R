testthat::test_that("Inferno evidence requires every applicable upstream test exactly once", {
  for (style in c("public", "header", "private_key_jwt")) {
    ...
    results <- lapply(expected, function(id) {
      list(test_id = id, result = "pass")
    })
    testthat::expect_true(inferno_verification_summary(results, expected)[[
      "passed"
    ]])
    testthat::expect_false(inferno_verification_summary(list(), expected)[[
      "passed"
    ]])
    testthat::expect_false(inferno_verification_summary(
      results[-1L],
      expected
    )[["passed"]])
    testthat::expect_false(inferno_verification_summary(
      c(results, results[1L]),
      expected
    )[["passed"]])
    duplicate <- results
    duplicate[[1L]] <- duplicate[[2L]]
    testthat::expect_false(inferno_verification_summary(duplicate, expected)[[
      "passed"
    ]])
    unrelated <- results
    unrelated[[1L]][["test_id"]] <- "different_test"
    testthat::expect_false(inferno_verification_summary(unrelated, expected)[[
      "passed"
    ]])
    ...
    testthat::expect_false(inferno_verification_summary(
      c(unfinished, list(list(test_suite_id = inferno_suite, result = "pass"))),
      expected
    )[["passed"]])
  }
})
...
testthat::test_that("successful token use before refresh cannot mask stale or missing use afterwards", {
  ...
  request <- function(
    ...
    list(
      ...
      request_headers = if (is.null(token)) {
        list()
      } else {
        list(list(name = "Authorization", value = paste("Bearer", token)))
      }
    )
  }
  response <- function(token) {
    jsonlite::toJSON(
      list(access_token = token, scope = "patient/Patient.rs"),
      auto_unbox = TRUE
    )
  }
  history <- list(
    request(
      1,
      paste0(
        "/auth/authorization?aud=",
        utils::URLencode(stack[["fhir_base"]], reserved = TRUE),
    ...
    request(
      ...
      response = response("synthetic-initial")
    ),
    request(3, "/fhir/Patient/patient-b", token = "synthetic-initial"),
    request(
      4,
      "/fhir/Practitioner/practitioner-b",
      token = "synthetic-initial"
    ),
    ...
    request(
      ...
      response = response("synthetic-refreshed")
    ),
