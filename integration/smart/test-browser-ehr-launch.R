for (async_mode in async_modes) {
  for (callback_mode in response_modes) {
    testthat::test_that(
      paste(
        "EHR browser launch under strict CSP",
        callback_mode,
        if (async_mode) "mirai" else "sync"
      ),
      {
        f <- retention_browser_setup(
          ...
          app_script = "integration/smart/fixture-ehr-app.R",
          app_function = "smart_ehr_fixture_app"
        )
        ...
        context_id <- f[["chrome"]][["Target"]][["getTargetInfo"]](
          targetId = browser[["get_target_id"]]()
        )[["targetInfo"]][["browserContextId"]]
        testthat::expect_true(
          is.character(context_id) &&
            length(context_id) == 1L &&
            nzchar(context_id)
        )
        target_id <- f[["chrome"]][["Target"]][["createTarget"]](
          "about:blank",
        ...
        testthat::expect_true(retention_browser_value(
          browser,
        ...
        testthat::expect_false(retention_browser_value(
          second,
        ...
        testthat::expect_length(
          retention_browser_snapshot(second)[["connections"]],
          2L
        )
