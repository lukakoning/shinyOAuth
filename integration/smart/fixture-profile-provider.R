...
smart_profile_provider <- function(
  site,
  callback,
  registration,
  launch,
  ...
  for (name in c("launches", "codes", "access", "refresh", "assertions")) {
  ...
  state[["metrics"]] <- list(
    ...
    authorization_body_bytes = 0L
  )
  lifetime <- if (is.null(behavior[["access_lifetime"]])) {
    3600
  } else {
    behavior[["access_lifetime"]]
  }
  state[["revoked"]] <- FALSE
  ...
  public <- jsonlite::fromJSON(
    jose::write_jwk(openssl::read_key(signing_pem)[["pubkey"]]),
  ...
  for (field in c("n", "e")) {
    ...
    public[[field]] <- sub(
      "=+$",
      "",
      chartr("+/", "-_", openssl::base64_encode(bytes))
    )
  }
  ...
  public[["use"]] <- "sig"
  initial_scopes <- c(
    if (launch == "ehr") "launch" else "launch/patient",
    "patient/Patient.rs",
    "user/Practitioner.r",
    "offline_access",
    "openid",
  ...
  if (ordinary_oidc) {
    initial_scopes <- c("openid", "profile", "offline_access")
  }
  random <- function() {
    unclass(as.character(openssl::sha256(openssl::rand_bytes(32))))
  }
  base <- function(req) {
    paste0(if (https) "https://" else "http://", req[["get_header"]]("Host"))
  }
  ...
  app[["get"]]("/fhir/.well-known/smart-configuration", function(req, res) {
    ...
    res[["send_json"]](
      list(
        issuer = paste0(origin, "/fhir"),
        jwks_uri = paste0(origin, "/keys"),
        authorization_endpoint = paste0(
          origin,
          "/authorize",
          if (authorization_method == "POST") {
            "?tenant=fixture%2Bvalue%26kept"
          } else {
            ""
          }
        ),
        ...
        response_modes_supported = list("query", "form_post"),
  ...
  app[["get"]]("/keys", function(req, res) {
    res[["send_json"]](list(keys = list(public)), auto_unbox = TRUE)
  })
  app[["get"]]("/ehr-launch", function(req, res) {
    id <- random()
    state[["launches"]][[id]] <- TRUE
    app_origin <- sub("/callback/.*$", "", callback)
    res[["set_status"]](302L)[["set_header"]](
      "Location",
      paste0(
        app_origin,
        "/launch?iss=",
        utils::URLencode(paste0(base(req), "/fhir"), reserved = TRUE),
        "&launch=",
        id
      )
    )[["send"]]("")
  ...
  authorize <- function(req, res) {
    ...
    if (
      post &&
        (!identical(req[["query"]], list(tenant = "fixture+value&kept")) ||
          !startsWith(
    ...
    scopes <- if (is.character(query[["scope"]])) {
      strsplit(query[["scope"]], " ", fixed = TRUE)[[1L]]
    } else {
      character()
    }
    valid_launch <- if (launch == "ehr") {
      is.character(query[["launch"]]) &&
        isTRUE(state[["launches"]][[query[["launch"]]]])
    } else {
      is.null(query[["launch"]])
    }
    if (
      !valid_launch ||
    ...
    if (post) {
      state[["metrics"]][["authorization_body_bytes"]] <- as.integer(req[[
        "get_header"
      ]]("Content-Length"))
    }
    id <- random()
    ...
    res[["set_type"]]("text/html")[["send"]](paste0(
      ...
      id,
      '">Approve synthetic access</a>',
      '<a id="deny" href="/deny?ticket=',
      id,
      '">Decline synthetic access</a></body></html>'
    ))
  }
  ...
  app[["get"]]("/approve", function(req, res) {
    ...
    if (is.null(record)) {
      return(res[["set_status"]](400L)[["send"]]("Unavailable"))
    }
  ...
  app[["post"]]("/token", function(req, res) {
    if (!is.null(res[["locals"]][["delayed_token"]])) {
      state[["metrics"]][["refresh_responses"]] <- state[["metrics"]][[
        "refresh_responses"
      ]] +
        1L
      return(res[["send_json"]](
        res[["locals"]][["delayed_token"]],
        auto_unbox = TRUE
      ))
    }
    body <- req[["form"]]
    ...
    authenticated <- switch(
      registration[["style"]],
      public = identical(body[["client_id"]], site) &&
        is.null(header) &&
        is.null(body[["client_assertion"]]),
      header = identical(
        header,
        paste0(
          "Basic ",
          openssl::base64_encode(charToRaw(paste0(
            site,
            ":",
            registration[["secret"]]
          )))
      ...
      private_key_jwt = tryCatch(
        {
          ...
          stopifnot(
            identical(jwt_header[["alg"]], registration[["assertion_alg"]]),
            identical(jwt_header[["kid"]], "fixture-client")
          )
          claims <- jose::jwt_decode_sig(
            body[["client_assertion"]],
            openssl::read_pubkey(registration[["public_pem"]])
          )
          now <- as.numeric(Sys.time())
          stopifnot(
            identical(claims[["iss"]], site),
            identical(claims[["sub"]], site),
            identical(claims[["aud"]], paste0(base(req), "/token")),
            claims[["exp"]] > now,
            claims[["exp"]] <= now + 300,
            is.character(claims[["jti"]]),
            is.null(state[["assertions"]][[claims[["jti"]]]])
          )
          state[["assertions"]][[claims[["jti"]]]] <- TRUE
          state[["metrics"]][["assertions"]] <- state[["metrics"]][[
            "assertions"
          ]] +
            1L
          TRUE
    ...
    if (initial) {
      ...
      grant <- list(
        revision = 1L,
        nonce = record[["query"]][["nonce"]],
        patient = paste0("synthetic-", site)
      )
      scopes <- initial_scopes
      if (identical(behavior[["consent"]], "reduced")) {
        scopes[scopes == "patient/Patient.rs"] <- "patient/Patient.r"
      }
      if (identical(behavior[["consent"]], "no_refresh")) {
        scopes <- setdiff(scopes, "offline_access")
      }
      if (identical(behavior[["consent"]], "missing_required")) {
        scopes <- setdiff(scopes, "patient/Patient.rs")
      }
      grant[["original_scopes"]] <- scopes
    } else if (identical(body[["grant_type"]], "refresh_token")) {
      state[["metrics"]][["refresh_attempts"]] <- state[["metrics"]][[
        "refresh_attempts"
      ]] +
        1L
      if (state[["revoked"]]) {
      ...
      if (
        !length(scopes) ||
          ...
          (!ordinary_oidc &&
            !is.null(body[["scope"]]) &&
            setequal(scopes, grant[["original_scopes"]]))
      ) {
        return(res[["set_status"]](400L)[["send_json"]](
      ...
      if (isTRUE(behavior[["change_patient"]])) {
        grant[["patient"]] <- paste0("synthetic-", site, "-new")
      }
      state[["metrics"]][["refreshes"]] <- state[["metrics"]][["refreshes"]] +
        1L
      if (!is.null(body[["scope"]])) {
        state[["metrics"]][["scoped_refreshes"]] <- state[["metrics"]][[
          "scoped_refreshes"
        ]] +
          1L
      }
    ...
    if (initial) {
      ...
      claims <- jose::jwt_claim(
        iss = paste0(base(req), "/fhir"),
        sub = paste0("clinician-", site),
        aud = site,
        exp = as.numeric(Sys.time()) + 300,
        iat = as.numeric(Sys.time()),
        nonce = grant[["nonce"]],
        fhirUser = paste0("Practitioner/clinician-", site)
      )
      if (ordinary_oidc) {
        token[["patient"]] <- token[["encounter"]] <- token[[
          "need_patient_banner"
        ]] <- NULL
        claims[["fhirUser"]] <- NULL
      }
      token[["id_token"]] <- jose::jwt_encode_sig(
        claims,
        openssl::read_key(signing_pem),
        header = list(kid = "fixture-id")
      )
    }
    if (!initial && isTRUE(behavior[["change_patient"]])) {
      token[["patient"]] <- grant[["patient"]]
    }
  ...
  current <- function(req) {
    ...
    if (!is.character(bearer) || !startsWith(bearer, "Bearer ")) {
      return(NULL)
    }
    grant <- state[["access"]][[substring(bearer, 8L)]]
    if (!is.null(grant) && grant[["expires"]] <= as.numeric(Sys.time())) {
      state[["metrics"]][["expired_reads"]] <- state[["metrics"]][[
        "expired_reads"
      ]] +
        1L
      return(NULL)
  ...
  app[["get"]]("/userinfo", function(req, res) {
    if (!ordinary_oidc || is.null(current(req))) {
      return(res[["set_status"]](401L)[["send"]]("Denied"))
    }
    res[["send_json"]](
      list(sub = paste0("clinician-", site), name = "Synthetic OIDC Name"),
      auto_unbox = TRUE
    )
  })
  app[["get"]]("/api/records", function(req, res) {
    grant <- current(req)
    if (!ordinary_oidc || is.null(grant)) {
      return(res[["set_status"]](401L)[["send"]]("Denied"))
    }
    res[["send_json"]](
      list(site = site, revision = grant[["revision"]]),
      auto_unbox = TRUE
    )
  })
  app[["get"]]("/fhir/Patient/:id", function(req, res) {
    ...
    if (
      is.null(grant) ||
        !identical(req[["params"]][["id"]], grant[["patient"]]) ||
        !any(
          c("patient/Patient.rs", "patient/Patient.r") %in% grant[["scopes"]]
        )
    ) {
      return(res[["set_status"]](403L)[["send"]]("Denied"))
    }
    ...
    res[["send_json"]](
      list(
        ...
        id = req[["params"]][["id"]],
        fixture_site = site,
        fixture_revision = grant[["revision"]]
      ),
      auto_unbox = TRUE
    )
  })
  app[["get"]]("/fhir/Practitioner/:id", function(req, res) {
    ...
    if (
      is.null(grant) ||
        !identical(req[["params"]][["id"]], paste0("clinician-", site)) ||
        !"user/Practitioner.r" %in% grant[["scopes"]]
    ) {
      return(res[["set_status"]](403L)[["send"]]("Denied"))
    }
  ...
  app[["get"]]("/fhir/Patient", function(req, res) {
    grant <- current(req)
    if (is.null(grant) || !"patient/Patient.rs" %in% grant[["scopes"]]) {
      return(res[["set_status"]](403L)[["send"]]("Denied"))
    }
    state[["metrics"]][["searches"]] <- state[["metrics"]][["searches"]] + 1L
    res[["send_json"]](
      list(resourceType = "Bundle", type = "searchset"),
      auto_unbox = TRUE
    )
  })
