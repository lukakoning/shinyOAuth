...
inferno_browser_app <- function(
  ...
  extra_scopes <- if (authorization_method == "POST") {
    paste0(
      "patient/Observation.rs?code=https://example.test/synthetic-codes|observation-",
      seq_len(32)
    )
  } else {
    character()
  ...
  clients <- lapply(sites, function(site) {
    ...
    args <- list(
      discovery = shinyOAuth::smart_discover(registration[["fhir_base"]]),
      client_id = registration[["client_id"]],
      redirect_uri = paste0(origin, "/callback/", site),
      scopes = c(
        if (launch == "standalone") "launch/patient",
        "patient/Patient.rs",
        paste0("user/", registration[["user_type"]], ".r"),
        "offline_access",
        extra_scopes
      ),
      required_scopes = c(
      ...
      token_auth_style = registration[["style"]],
      authorization_method = authorization_method,
      authorization_server_mode = if (length(sites) > 1L) {
        "multi_redirect_uri"
      } else {
        "single"
      },
      authorization_server_redirect_uris = if (length(sites) > 1L) {
        callbacks
      } else {
        character()
      }
    ...
    if (registration[["style"]] == "header") {
      args[["client_secret"]] <- registration[["secret"]]
    }
    if (registration[["style"]] == "private_key_jwt") {
      args[["client_assertion_private_key"]] <- openssl::read_key(registration[[
        "private_pem"
      ]])
  ...
  ui <- shiny::fluidPage(
    ...
    lapply(sites, function(site) {
      shiny::tagList(lapply(
        c("connect", "read", "user", "search", "refresh", "disconnect"),
        function(action) {
          shiny::actionButton(paste0(action, "_", site), paste(action, site))
        }
      ))
    }),
    shiny::actionButton("narrow_a", "Narrow A"),
    shiny::actionButton("widen_a", "Try broader scopes"),
  ...
  server <- function(input, output, session) {
    ...
    for (site in sites) {
      local({
        ...
        shiny::observeEvent(
          input[[paste0("user_", selected)]],
          perform(paste0("user_", selected), function() {
            body <- httr2::resp_body_json(shinyOAuth::smart_fhir_user(connection(
              selected
            )))
            stopifnot(
              identical(body[["resourceType"]], expected[["user_type"]]),
              identical(body[["id"]], expected[["user_id"]])
            )
            if (expected[["user_type"]] != "Patient") {
              stopifnot(!identical(body[["id"]], expected[["patient"]]))
            }
            TRUE
          })
        )
        shiny::observeEvent(
          ...
          perform(paste0("refresh_", selected), function() {
            connection(selected)[["refresh"]]()
          })
        )
        shiny::observeEvent(
          input[[paste0("search_", selected)]],
          perform(paste0("search_", selected), function() {
            response <- connection(selected)[["request"]](
              "fhir",
              "Patient",
              required_scopes = "patient/Patient.s"
            )
            body <- httr2::resp_body_json(response)
            stopifnot(
              identical(body[["resourceType"]], "Bundle"),
              identical(body[["type"]], "searchset")
            )
            TRUE
          })
        )
        shiny::observeEvent(
          input[[paste0("disconnect_", selected)]],
          health[["disconnect"]](
            connection(selected)[["summary"]]()[["connection_id"]],
            revoke = FALSE
          )
        )
      })
    ...
    shiny::observeEvent(
      ...
      perform("narrow_a", function() {
        connection("a")[["refresh"]](scopes = limited)
      })
    )
    shiny::observeEvent(
      input[["widen_a"]],
      perform("widen_a", function() {
        connection("a")[["refresh"]](scopes = c(limited, "patient/Patient.s"))
      })
    )
  ...
  routes <- if (launch == "ehr") {
    list(shinyOAuth::smart_launch_route("/launch", sites))
  } else {
    list()
  }
