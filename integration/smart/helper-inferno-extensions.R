inferno_identity_case <- function(root, output, stack, row, index) {
  ...
  registration <- inferno_registration(stack, "public", "RS384", "a", row$user_type, "synthetic-encounter")
  app <- inferno_start_app(root, directory, list(a = registration), row$launch, row$async)
  session <- inferno_begin_session(stack, registration, app$origin, "a", row$launch)
  chrome <- inferno_open_browser(app)
  browser <- chrome$browser
  if (row$launch == "ehr") browser$Page$navigate(session$launch_url) else
  ...
  stopifnot(identical(first$connections[[1L]]$connection_id, retained$connections[[1L]]$connection_id))
  for (id in c("read_a", "user_a")) retention_browser_action(browser, id, paste0(id, ":ok"))
  ...
  list(user_type = row$user_type, launch = row$launch, async = row$async,
    browser = chrome$version, encounter_preserved = TRUE, identity_and_patient_checked = TRUE,
    retained_after_refresh = TRUE, verification = verification, exchanges = exchanges)
}
...
inferno_identity_exchanges <- function(stack, session, registration) {
  ...
  user <- paste0(base, "/", registration$user_type, "/", registration$user_id)
  for (index in seq_along(tokens)) {
    token <- tokens[[index]]
    params <- shiny::parseQueryString(token$request_body)
    stopifnot(token$status == 200L, identical(params$grant_type,
    ...
    following <- Filter(function(request) request$index > token$index && request$index < end, reads)
    expected <- if (index == 1L) 1L else 2L
    stopifnot(length(following) == 2L * expected,
      all(vapply(following, path, character(1)) %in% c(patient, user)))
    if (patient != user) for (wanted in c(patient, user))
      stopifnot(sum(vapply(following, function(request) path(request) == wanted, logical(1))) == expected)
  ...
  list(code_exchanges = 1L, refreshes = 1L, resource_reads = 6L, every_read_uses_current_token = TRUE)
}
