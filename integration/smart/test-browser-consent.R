for (async in c(FALSE, TRUE)) {
  for (consent in c("reduced", "no_refresh", "missing_required", "denied")) {
    testthat::test_that(paste("SMART consent", consent, "async", async), {
      f <- lifecycle_setup(
        async,
        list(
          consent = consent,
          access_lifetime = if (consent == "no_refresh") 10 else 3600
        ),
      ...
      retention_browser_click(
        f[["browser"]],
        if (consent == "denied") "deny" else "approve"
      )
      ...
      testthat::expect_identical(
        lifecycle_connection(f[["browser"]], "b")[["connection_id"]],
        b_id
      )
      if (refused) {
        testthat::expect_gt(
          length(retention_browser_snapshot(f[["browser"]])[["errors"]]),
          0L
        )
        retention_browser_action(f[["browser"]], "read_a", "unavailable")
        testthat::expect_identical(
          f[["metrics"]]()[["exchanges"]],
          if (consent == "denied") 0L else 1L
        )
        testthat::expect_identical(
          f[["metrics"]]()[["denials"]],
          if (consent == "denied") 1L else 0L
        )
