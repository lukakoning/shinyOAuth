permissions_app <- function(origin, listen_port, fhir_base, ca, async) {
  ...
  client <- shinyOAuth::smart_client(shinyOAuth::smart_discover(fhir_base),
    client_id = "permissions", redirect_uri = paste0(origin, "/callback"),
    scopes = c("launch/patient", "patient/Patient.rs", "offline_access"),
    required_scopes = "patient/Patient.r", identity = "fhirUser", token_auth_style = "public", label = "FHIR")
  manager <- shinyOAuth::oauth_connections(list(a = client), origin,
    retention = "browser", owner = shinyOAuth::oauth_browser_owner(),
  ...
  ui <- shiny::fluidPage(shinyOAuth::use_shinyOAuth(),
    lapply(c("connect", "read", "user", "search", "other", "observation", "narrow"), function(id)
  ...
  server <- function(input, output, session) {
    ...
    perform <- function(id, fn) {
      ...
      if (inherits(value, "promise")) promises::then(value,
        function(...) finish("ok"), function(...) finish("unavailable")) else finish(value)
    }
