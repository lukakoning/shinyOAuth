run_inferno_extensions <- function(args = commandArgs(trailingOnly = TRUE)) {
  if (length(args) != 1L || !args %in% "identity")
    stop("Usage: Rscript integration/smart/run-inferno-extensions.R identity")
  root <- normalizePath(".", winslash = "/")
  for (file in c("helper-inferno-evidence.R", "helper-inferno.R", "helper-inferno-browser.R",
    "helper-inferno-exchanges.R", "helper-inferno-extensions.R"))
