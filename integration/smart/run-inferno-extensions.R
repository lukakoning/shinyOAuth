run_inferno_extensions <- function(args = commandArgs(trailingOnly = TRUE)) {
  if (length(args) != 1L || !args %in% c("identity", "account")) {
    stop(
      "Usage: Rscript integration/smart/run-inferno-extensions.R identity|account"
    )
  }
  root <- normalizePath(".", winslash = "/")
  for (file in c(
    "helper-inferno-evidence.R",
    "helper-inferno.R",
    "helper-inferno-browser.R",
    "helper-inferno-exchanges.R",
    "helper-inferno-extensions.R",
    "helper-inferno-account.R"
  )) {
  ...
  output <- file.path(
    ...
    paste0("inferno-", args, "-", format(Sys.time(), "%Y%m%d-%H%M%S"))
  )
  ...
  if (args == "account") {
    ...
    for (index in 1:2) {
      evidence[["scenarios"]][[index]] <-
        inferno_account_case(
          root,
          output,
          list(a = stack, b = second),
          index == 2L,
          index
        )
    }
    stopifnot(
      length(evidence[["scenarios"]]) == 2L,
      all(vapply(
        evidence[["scenarios"]],
        function(row) isTRUE(row[["passed"]]),
        logical(1)
      ))
    )
  ...
  cases <- expand.grid(
    user_type = c("Patient", "RelatedPerson"),
  ...
  for (index in seq_len(nrow(cases))) {
    ...
    evidence[["scenarios"]][[index]] <- inferno_identity_case(
      root,
      output,
      stack,
      row,
  ...
  stopifnot(
    length(evidence[["scenarios"]]) == 8L,
    all(vapply(
      evidence[["scenarios"]],
      function(row) isTRUE(row[["verification"]][["passed"]]),
      logical(1)
    ))
  )
  evidence[["status"]] <- "passed"
