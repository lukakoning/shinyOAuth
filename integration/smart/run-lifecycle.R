...
run_smart_lifecycle <- function(args = commandArgs(trailingOnly = TRUE)) {
  if (
    length(args) != 1L ||
      !args %in% c("expiry", "context", "interrupted", "consent")
  ) {
    stop(
      "Usage: Rscript integration/smart/run-lifecycle.R expiry|context|interrupted|consent"
    )
  }
  scenario <- args[[1L]]
  for (package in c(
  ...
  for (path in c(
    ...
    "integration/smart/fixture-profile-provider.R",
    "integration/smart/helper-lifecycle.R"
  )) {
  ...
  evidence <- list(
    status = "failed",
    scenario = scenario,
    server = "synthetic SMART fixture",
    external_interoperability = "not_established",
    token_transport = c("sync", "mirai")
  )
  on.exit(
    jsonlite::write_json(
      evidence,
  ...
  results <- testthat::test_file(
    paste0("integration/smart/test-browser-", scenario, ".R"),
    env = environment(),
    reporter = "summary",
  ...
  if (
    nrow(counts) != expected_scenarios ||
      evidence[["tests"]][["passed"]] == 0L ||
      any(unlist(evidence[["tests"]][c("failed", "error", "skipped")]) != 0)
  ) {
    stop("SMART lifecycle gate failed")
  }
