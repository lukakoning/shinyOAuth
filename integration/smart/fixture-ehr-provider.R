...
smart_ehr_fixture_provider <- function(site, callback) {
  ...
  for (name in c("launches", "codes", "access", "refresh")) {
  ...
  app[["get"]]("/authorize", function(req, res) {
    query <- req[["query"]]
    launch <- if (is.character(query[["launch"]])) {
      state[["launches"]][[query[["launch"]]]]
    } else {
      NULL
    }
    if (
      is.null(launch) ||
        launch[["expires"]] <= as.numeric(Sys.time()) ||
        ...
        !all(
          c("launch", "patient/Patient.r", "online_access") %in%
            strsplit(query[["scope"]], " ", fixed = TRUE)[[1L]]
        )
    ) {
      return(res[["set_status"]](400L)[["send"]](
        "Invalid SMART fixture authorization"
      ))
    }
    ...
    state[["metrics"]][["authorizations"]] <- state[["metrics"]][[
      "authorizations"
    ]] +
      1L
  ...
  app[["post"]]("/token", function(req, res) {
    ...
    token <- list(
      ...
      token_type = "Bearer",
      expires_in = 3600,
      scope = "launch patient/Patient.r online_access"
    )
    if (initial) {
  ...
  app[["get"]]("/fhir/Patient/:id", function(req, res) {
    ...
    grant <- if (is.character(bearer) && length(bearer) == 1L) {
      state[["access"]][[bearer]]
    } else {
      NULL
    }
    if (
      is.null(grant) || !identical(req[["params"]][["id"]], grant[["patient"]])
    ) {
