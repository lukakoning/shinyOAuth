run_smart_mixed <- function() {
  packages <- c("shinyOAuth", "shiny", "webfakes", "httpuv", "chromote", "callr", "testthat",
    "httr2", "jsonlite", "curl", "withr", "mirai", "promises", "jose", "openssl", "processx")
  ...
  cases <- expand.grid(managed_oidc = c(FALSE, TRUE), response_mode = c("query", "form_post"),
    async = c(FALSE, TRUE), stringsAsFactors = FALSE)
  output <- file.path("integration/smart/.artifacts", paste0("mixed-", format(Sys.time(), "%Y%m%d-%H%M%S")))
  dir.create(output, recursive = TRUE)
  evidence <- list(gate = "ordinary OIDC and SMART browser composition", status = "failed",
