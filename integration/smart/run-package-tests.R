...
totals <- as.list(colSums(counts[c(
  "failed",
  "error",
  "warning",
  "skipped",
...
allowed <- .Platform[["OS.type"]] == "windows" &
  skips[["file"]] == "test-state-store-concurrent-replay.R" &
  skips[["test"]] ==
    "atomic $take() prevents concurrent replay across parallel workers"
passed <- totals[["failed"]] == 0 && totals[["error"]] == 0 && all(allowed)
output <- file.path(
  "integration/smart/.artifacts",
  paste0("package-", format(Sys.time(), "%Y%m%d-%H%M%S"))
)
...
if (!passed) {
  stop("Package/browser tests failed or unexpectedly skipped")
}
