for (index in seq_len(nrow(cases))) {
  row <- cases[index, ]
  testthat::test_that(
    paste("SMART profile", paste(unlist(row), collapse = " ")),
    {
      key <- if (identical(row[["assertion_alg"]], "ES384")) {
        openssl::ec_keygen("P-384")
      } else {
        openssl::rsa_keygen(2048)
      }
      registration <- list(
        style = row[["registration"]],
        assertion_alg = row[["assertion_alg"]],
        secret = "synthetic-profile-secret",
        public_pem = openssl::write_pem(key[["pubkey"]]),
        private_pem = openssl::write_pem(key)
      )
      provider_registration <- registration
      provider_registration[["private_pem"]] <- NULL
      provider_factory <- function(site, callback) {
        smart_profile_provider(
          site,
          callback,
      ...
      f <- retention_browser_setup(
        ...
        app_script = "integration/smart/fixture-profile-app.R",
        app_function = "smart_profile_app",
        https = cross_site,
        https_providers = cross_site,
        app_args = list(
      ...
      metrics <- function(site) {
        httr2::request(paste0(f[["bases"]][[site]], "/metrics")) |>
          httr2::req_timeout(5) |>
          httr2::req_perform() |>
      ...
      authorize <- function(site, count) {
        ...
        if (cross_site) {
          testthat::expect_identical(
            retention_browser_value(browser, "location.origin"),
            f[["bases"]][[site]]
          )
          ...
          testthat::expect_true(all(vapply(
            cookies,
            function(cookie) isTRUE(cookie[["secure"]]),
            logical(1)
          )))
          testthat::expect_true(any(vapply(
            cookies,
            function(cookie) {
              identical(cookie[["sameSite"]], "Lax") &&
                isTRUE(cookie[["httpOnly"]])
            },
            logical(1)
          )))
        }
      ...
      retained_a <- Filter(
        function(value) identical(value[["client_label"]], "Site a"),
        both[["connections"]]
      )[[1L]]
      ...
      for (site in c("a", "b")) {
        ...
        testthat::expect_identical(
          value[["authorization_posts"]],
          if (authorization_method == "POST") 1L else 0L
        )
        testthat::expect_identical(
          value[["authorization_gets"]],
          if (authorization_method == "GET") 1L else 0L
        )
        if (authorization_method == "POST") {
          testthat::expect_gt(value[["authorization_body_bytes"]], 8192L)
        }
        testthat::expect_identical(
          value[["assertions"]],
          if (row[["registration"]] == "private_key_jwt") {
            1L + value[["refreshes"]]
          } else {
            0L
          }
      ...
      retention_browser_wait(
        ...
        function() {
          ...
          b <- Filter(
            function(value) identical(value[["client_label"]], "Site b"),
            rows
          )
