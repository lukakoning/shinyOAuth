...
run_inferno <- function(args = commandArgs(trailingOnly = TRUE)) {
  if (!all(args %in% "--quick")) {
    stop("Usage: Rscript integration/smart/run-inferno.R [--quick]")
  }
  packages <- c(
    "shinyOAuth",
    "shiny",
  ...
  output <- file.path(
    ...
    paste0("inferno-", format(Sys.time(), "%Y%m%d-%H%M%S"))
  )
  dir.create(output, recursive = TRUE)
  for (file in c(
    "integration/smart/helper-inferno-evidence.R",
    "integration/smart/helper-inferno.R",
    "integration/smart/helper-inferno-browser.R",
    "integration/smart/helper-inferno-exchanges.R",
    "integration/connections/helper-browser.R"
  )) {
    source(file, local = TRUE)
  }
  testthat::test_file(
    file.path(root, "integration/smart/test-inferno-evidence.R"),
    env = environment(),
    reporter = "summary",
  ...
  evidence <- list(
    status = "failed",
    suite_id = inferno_suite,
    complete_matrix = !"--quick" %in% args,
    simulator_modified = TRUE,
    unmodified_external_interoperability = "not_established",
    max_id_token_lifetime_seconds = 366 * 86400,
    checkout_revision = trimws(processx::run("git", c("rev-parse", "HEAD"))[[
      "stdout"
    ]]),
    checkout_dirty = nzchar(trimws(processx::run(
      "git",
      c("status", "--porcelain")
    )[["stdout"]])),
    ...
    response_mode = "query",
    limitations = list(
      form_post_callback = "not_verified_by_inferno",
      server_permission_enforcement = "not_verified",
      refresh_token_rotation = "not_verified_by_inferno",
      cross_vendor_portability = "not_established"
    ),
    scenarios = list()
  )
  on.exit(
    {
      paths <- c(
        ...
        Sys.getenv("SHINYOAUTH_INFERNO_REPORT")
      )
  ...
  run_env <- environment()
  stacks <- lapply(c("a", "b"), function(site) {
    inferno_start(root, output, site, .env = run_env)
  })
  names(stacks) <- c("a", "b")
  ...
  if ("--quick" %in% args) {
    cases <- cases[
      cases[["profile"]] == "public" & cases[["launch"]] == "standalone",
      ,
      drop = FALSE
    ]
  }
  run_case <- function(row, index) {
    ...
    registrations <- lapply(names(stacks), function(site) {
      inferno_registration(stacks[[site]], style, algorithm, site)
    })
    ...
    authorize <- function(site, count) {
      ...
      if (row[["launch"]] == "ehr") {
        stopifnot(
          is.character(session[["launch_url"]]),
          startsWith(
            session[["launch_url"]],
            paste0(app[["origin"]], "/launch?")
          )
        )
      ...
      value <- retention_browser_wait(
        browser,
        function() {
          value <- retention_browser_snapshot(browser)
          if (!is.null(value) && length(value[["connections"]]) == count) {
            value
          } else {
            NULL
          }
    ...
    action <- function(id, ok = TRUE, target = browser) {
      retention_browser_action(
        target,
        id,
        paste0(id, if (ok) ":ok" else ":unavailable")
      )
    }
    connection <- function(snapshot, site) {
      rows <- Filter(
        function(value) identical(value[["client_label"]], paste("Site", site)),
    ...
    for (id in c("read_a", "user_a", "search_a", "narrow_a")) {
      action(id)
    }
    ...
    both <- authorize("b", 2L)
    stopifnot(
      both[["session"]] > first[["session"]],
      identical(
        connection(first, "a")[["connection_id"]],
        connection(both, "a")[["connection_id"]]
      ),
      identical(connection(both, "a")[["status"]], "limited")
    )
    for (id in c("read_b", "user_b", "search_b")) {
      action(id)
    }
    ...
    retained <- retention_browser_wait(
      ...
      function() {
        ...
        if (
          !is.null(value) &&
            value[["session"]] > both[["session"]] &&
            length(value[["connections"]]) == 2L
        ) {
          value
        } else {
          NULL
    ...
    for (site in names(stacks)) {
      stopifnot(identical(
        connection(both, site)[["connection_id"]],
        connection(retained, site)[["connection_id"]]
      ))
    }
    stopifnot(
      identical(connection(retained, "a")[["status"]], "limited"),
      identical(connection(retained, "b")[["status"]], "active")
    )
    for (id in c("read_a", "user_a")) {
      action(id)
    }
    for (id in c("search_a", "widen_a")) {
      action(id, FALSE)
    }
    for (id in c(
      "refresh_a",
    ...
    stopifnot(
      length(retention_browser_snapshot(foreign)[["connections"]]) == 0L
    )
    ...
    retention_browser_wait(
      ...
      function() {
        identical(
          connection(retention_browser_snapshot(browser), "b")[["status"]],
          "disconnected"
        )
      },
      "disconnect site B"
    )
    for (id in c("read_b", "user_b", "refresh_b")) {
      action(id, FALSE)
    }
    for (id in c("read_a", "user_a")) {
      action(id)
    ...
    verification <- lapply(names(stacks), function(site) {
      ...
      verified[["driver_exchange_checks"]] <- inferno_exchange_summary(
        stacks[[site]],
        sessions[[site]],
        registrations[[site]],
        row[["launch"]],
    ...
    list(
      ...
      app_flow = "passed",
      patient_read = TRUE,
      validated_fhir_user = TRUE,
      refresh_and_read = TRUE,
      two_sites_retained = TRUE,
      browser_owner_isolation = TRUE,
      narrowing_retained = TRUE,
      independent_refresh_and_disconnect = TRUE,
      logout = TRUE,
      inferno = verification,
      status = if (
        all(vapply(verification, function(site) site[["passed"]], logical(1)))
      ) {
        "passed"
      } else {
        "failed"
  ...
  for (index in seq_len(nrow(cases))) {
    ...
    message(
      ...
      row[["authorization_method"]],
      " / ",
      if (row[["async"]]) "mirai" else "sync"
    )
    evidence[["scenarios"]][[index]] <- tryCatch(
      run_case(row, index),
      error = function(e) {
  ...
  if (
    length(evidence[["scenarios"]]) != nrow(cases) ||
      !all(vapply(
        evidence[["scenarios"]],
        function(row) identical(row[["status"]], "passed"),
        logical(1)
      ))
  ) {
    stop("Inferno application matrix did not pass")
  }
  ...
  cat(
    "Inferno application scenarios passed:",
    nrow(cases),
    "; simulator modifications recorded.\n"
  )
}
run_inferno()
