...
smart_ehr_fixture_app <- function(
  origin,
  providers,
  async = FALSE,
  response_mode = "query"
  ...
  clients <- lapply(c("a", "b"), function(site) {
    ...
    shinyOAuth::smart_client(
      discovery,
      site,
      paste0(origin, "/callback/", site),
      scopes = c("patient/Patient.r", "online_access"),
      launch = "ehr",
  ...
  base_ui <- shiny::fluidPage(
    ...
    shiny::actionButton("connect_a", "Reconnect A"),
  ...
  server <- function(input, output, session) {
    ...
    shiny::observeEvent(input[["refresh_a"]], {
      value <- tryCatch(connection("a")[["refresh"]](), error = function(...) {
        FALSE
      })
      if (inherits(value, "promise")) {
        promises::then(value, function(...) result("refreshed"), function(...) {
          result("unavailable")
        })
      } else {
        result(if (isTRUE(value)) "refreshed" else "unavailable")
      }
    })
  ...
  launch_ui <- shinyOAuth::oauth_connections_ui(
    base_ui,
    "health",
    manager,
