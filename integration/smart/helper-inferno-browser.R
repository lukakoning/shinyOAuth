inferno_registration <- function(
  ...
  registration <- list(
    ...
    client_id = paste0(
      ...
      site,
      "-",
      paste(openssl::sha256(openssl::rand_bytes(16)))
    ),
    patient = paste0("patient-", site),
    practitioner = paste0("practitioner-", site)
  )
  registration[["user_type"]] <- user_type
  registration[["user_id"]] <- if (user_type == "Patient") {
    registration[["patient"]]
  } else if (user_type == "Practitioner") {
    registration[["practitioner"]]
  } else {
    paste0("representative-", site)
  }
  ...
  if (style == "header") {
    registration[["secret"]] <- paste(
      format(openssl::rand_bytes(32)),
      collapse = ""
    )
  }
  if (style == "private_key_jwt") {
    key <- if (algorithm == "ES384") {
      openssl::ec_keygen("P-384")
    } else {
    ...
    for (field in intersect(c("n", "e"), names(jwk))) {
      encoded <- jwk[[field]]
      bytes <- openssl::base64_decode(paste0(
        chartr("-_", "+/", encoded),
    ...
    jwk[["alg"]] <- algorithm
    jwk[["use"]] <- "sig"
    registration[["jwks"]] <- jsonlite::toJSON(
      list(keys = list(jwk)),
      auto_unbox = TRUE
    )
  }
...
inferno_begin_session <- function(stack, registration, origin, site, launch) {
  ...
  session <- inferno_api(
    ...
    list(
      ...
      suite_options = list(list(
        id = "client_type",
        value = paste("SMART", "authorization_code", type, sep = ",")
      ))
    )
  )
  context <- list(patient = registration[["patient"]])
  if (!is.null(registration[["encounter"]])) {
    context[["encounter"]] <- registration[["encounter"]]
  }
  resources <- list(list(
    resource = list(resourceType = "Patient", id = registration[["patient"]])
  ))
  if (registration[["user_type"]] != "Patient") {
    resources <- c(
      ...
      list(list(
        resource = list(
          resourceType = registration[["user_type"]],
  ...
  inputs <- list(
    ...
    fhir_user_relative_reference = paste0(
      registration[["user_type"]],
      "/",
      registration[["user_id"]]
    ),
    echoed_fhir_response = jsonlite::toJSON(
      list(
        resourceType = "Bundle",
        type = "searchset",
        total = 0L,
    ...
    fhir_read_resources_bundle = jsonlite::toJSON(
      list(resourceType = "Bundle", type = "collection", entry = resources),
      auto_unbox = TRUE
    )
  )
  if (launch == "ehr") {
    inputs[["smart_launch_urls"]] <- paste0(origin, "/launch")
  }
  if (registration[["style"]] == "header") {
    inputs[["smart_client_secret"]] <- registration[["secret"]]
  }
  if (registration[["style"]] == "private_key_jwt") {
    inputs[["smart_jwk_set"]] <- registration[["jwks"]]
  }
  run <- inferno_api(
    stack,
    "/api/test_runs",
    list(
      ...
      test_suite_id = inferno_suite,
      inputs = lapply(names(inputs), function(name) {
        list(name = name, value = as.character(inputs[[name]]))
      })
    )
  )
  inferno_wait(
    function() {
      ...
      if (current[["status"]] == "done") {
        stop("Inferno finished before the application interaction")
      }
  ...
  data <- inferno_api(
    stack,
    paste0("/api/test_sessions/", session[["id"]], "/session_data")
  )
  ...
  list(
    id = session[["id"]],
    run_id = run[["id"]],
    expected_tests = inferno_expected_tests(registration[["style"]]),
...
inferno_finish_session <- function(stack, session) {
  stopifnot(
    is.character(session[["continuation"]]),
    startsWith(session[["continuation"]], paste0(stack[["origin"]], "/"))
  )
  inferno_api(
    stack,
    substring(session[["continuation"]], nchar(stack[["origin"]]) + 1L),
    json = FALSE
  )
  inferno_wait(
    function() {
      current <- inferno_api(
        stack,
        paste0("/api/test_runs/", session[["run_id"]])
      )
  ...
  results <- inferno_api(
    stack,
    paste0("/api/test_sessions/", session[["id"]], "/results")
  )
  ...
  process <- callr::r_bg(
    function(
      ...
      options(
        ...
        shinyOAuth.debug = identical(
          Sys.getenv("SHINYOAUTH_INFERNO_DEBUG"),
          "true"
        )
      )
    ...
    libpath = .libPaths(),
    supervise = TRUE,
    stdout = file.path(output, "app.stdout"),
    stderr = file.path(output, "app.stderr")
  )
  withr::defer(
    {
...
inferno_open_browser <- function(app, .env = parent.frame()) {
  ...
  inferno_wait(
    function() {
      if (!app[["process"]][["is_alive"]]()) {
        stop("Shiny app exited; inspect the private app.stderr log")
      }
      tryCatch(
        {
          inferno_api(app, "/", json = FALSE)
          TRUE
        },
        error = function(e) {
  ...
  tryCatch(
    retention_browser_value(
      ...
      paste0(
        ...
        jsonlite::toJSON(app[["origin"]], auto_unbox = TRUE),
        ")"
      )
    ),
    error = function(...) NULL
  ...
  list(
    browser = browser,
    chrome = chrome,
    version = chrome[["Browser"]][["getVersion"]]()[["product"]]
  )
}
