inferno_start_account_app <- function(
  root,
  output,
  registrations,
  async,
  ...
  process <- callr::r_bg(
    function(root, origin, port, registrations, async) {
      Sys.setenv(
      ...
      options(
        shinyOAuth.max_id_token_lifetime = 366 * 86400,
        shinyOAuth.allowed_hosts = c("localhost", "127.0.0.1")
      )
      source(file.path(root, "integration/connections/fixture-account-app.R"))
      account_fixture_app(
        origin,
        lapply(registrations, function(reg) reg[["fhir_base"]]),
        async,
        listen_port = port,
        smart_registrations = registrations
      )
    },
    args = list(root, tls[["origin"]], port, registrations, async),
    libpath = .libPaths(),
    supervise = TRUE,
...
inferno_account_case <- function(root, output, stacks, async, index) {
  ...
  registrations <- lapply(names(stacks), function(site) {
    inferno_registration(stacks[[site]], "public", "RS384", site)
  })
  ...
  logout <- function() {
    ...
    retention_browser_wait(
      browser,
      function() {
        identical(retention_browser_snapshot(browser)[["result"]], "login")
      },
      ...
      function() {
        ...
        if (
          length(value[["connections"]]) == 1L &&
            identical(value[["connections"]][[1L]][["client_label"]], site)
        ) {
          value
        } else {
          NULL
  ...
  context <- chrome[["chrome"]][["Target"]][["getTargetInfo"]](
    targetId = browser[["get_target_id"]]()
  )[["targetInfo"]][["browserContextId"]]
  target <- chrome[["chrome"]][["Target"]][["createTarget"]](
    "about:blank",
    browserContextId = context
  )[["targetId"]]
  old <- chromote::ChromoteSession[["new"]](
  ...
  retention_browser_wait(
    old,
    function() length(retention_browser_snapshot(old)[["connections"]]) == 1L,
    "old Alice tab"
  )
  logout()
  ...
  stopifnot(
    length(restored[["connections"]]) == 1L,
    identical(
      restored[["connections"]][[1L]][["connection_id"]],
      alice[["connections"]][[1L]][["connection_id"]]
    )
  )
  retention_browser_action(old, "read_a", "unavailable")
  ...
  stopifnot(
    ...
    identical(
      restored[["connections"]][[1L]][["connection_id"]],
      bob[["connections"]][[1L]][["connection_id"]]
    )
  )
  retention_browser_action(browser, "read_b", "b:patient")
  verification <- lapply(names(stacks), function(site) {
    ...
    result[["exchanges"]] <- inferno_account_exchanges(
      stacks[[site]],
      sessions[[site]],
      registrations[[site]],
      site == "a"
  ...
  list(
    ...
    restored_refresh = TRUE,
    other_account_survives_disconnect = TRUE,
...
inferno_account_exchanges <- function(stack, session, registration, refreshed) {
  ...
  stopifnot(
    length(tokens) == if (refreshed) 2L else 1L,
    length(reads) == 4L,
    all(vapply(
      tokens,
      function(request) request[["status"]] == 200L,
      logical(1)
    ))
  )
  stopifnot(identical(
    vapply(
      tokens,
      function(request) {
        shiny::parseQueryString(request[["request_body"]])[["grant_type"]]
      },
      character(1)
    ),
  ...
  expected <- c(
    paste0(base, "/Patient/", registration[["patient"]]),
  ...
  for (request in reads) {
    preceding <- Filter(
      function(token) token[["index"]] < request[["index"]],
      tokens
    )
  ...
  list(
    code_exchanges = 1L,
    refreshes = as.integer(refreshed),
    resource_reads = 4L,
