for (async in c(FALSE, TRUE)) {
  for (changed in c(FALSE, TRUE)) {
    testthat::test_that(
      paste("SMART refresh context", async, "changed", changed),
      {
        expected <- list(
          patient = if (changed) "synthetic-a-new" else "synthetic-a",
          encounter = if (changed) NULL else "encounter-a",
          revision = if (changed) 2L else 1L
        )
        f <- lifecycle_setup(
          async,
          list(change_patient = changed),
          list(expected_context = expected)
        )
        ...
        retention_browser_action(
          f[["browser"]],
          "read_a",
          paste0("a:2:context-", expected[["revision"]])
        )
        retention_browser_action(f[["browser"]], "user_a", "a:user")
        f[["browser"]][["Page"]][["navigate"]](paste0(
          f[["origin"]],
          "/retained"
        ))
