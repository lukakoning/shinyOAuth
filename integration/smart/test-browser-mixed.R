for (index in seq_len(nrow(cases))) {
  row <- cases[index, ]
  testthat::test_that(
    paste("mixed OIDC/SMART", paste(unlist(row), collapse = " ")),
    {
      provider_factory <- function(site, callback) {
        smart_profile_provider(
          site,
          callback,
          registration = list(style = "public"),
          launch = "standalone",
          https = TRUE,
          behavior = list(
            ordinary_oidc = site == "b",
            access_lifetime = if (site == "b" && !row[["managed_oidc"]]) {
              12
            } else {
              3600
            }
      ...
      f <- retention_browser_setup(
        ...
        app_script = "integration/smart/fixture-mixed-app.R",
        app_function = "smart_mixed_app",
        app_args = list(managed_oidc = row[["managed_oidc"]]),
        https = TRUE,
        https_providers = TRUE
      )
      ...
      authorize <- function(site) {
        ...
        retention_browser_wait(
          ...
          function() {
            ...
            if (site == "b") {
              return(if (isTRUE(value[["oidc_authenticated"]])) value else NULL)
            }
            rows <- Filter(
              function(x) {
                x[["client_label"]] == "SMART" && x[["status"]] == "active"
              },
              value[["connections"]]
            )
      ...
      first <- authorize("a")
      testthat::expect_identical(
        first[["oidc_authenticated"]],
        row[["managed_oidc"]]
      )
      id <- Filter(
        function(x) x[["client_label"]] == "SMART",
        first[["connections"]]
      )[[1]][["connection_id"]]
      if (!row[["managed_oidc"]]) {
        authorize("b")
      ...
      if (row[["managed_oidc"]]) {
        ...
        retention_browser_wait(
          ...
          function() {
            value <- retention_browser_snapshot(browser)
            !is.null(value) &&
              !is.null(value[["oidc_expiry"]]) &&
              value[["oidc_expiry"]] > expiry
          },
      ...
      retention_browser_wait(
        ...
        function() {
          ...
          any(vapply(
            rows,
            function(x) {
              x[["client_label"]] == "SMART" && x[["status"]] == "disconnected"
            },
            logical(1)
          ))
        },
      ...
      retention_browser_action(browser, "read_b", "b:2")
      retention_browser_action(browser, "identity_b", "identity:ok")
      testthat::expect_true(retention_browser_snapshot(browser)[[
        "oidc_authenticated"
      ]])
