for (async in c(FALSE, TRUE)) {
  for (revoked in c(FALSE, TRUE)) {
    testthat::test_that(
      paste("expired retained SMART grant", async, "revoked", revoked),
      {
        ...
        retention_browser_wait(
          f[["browser"]],
          function() as.numeric(Sys.time()) > expires + 1,
          "actual access-token expiry",
          timeout = 15
        )
        ...
        testthat::expect_identical(
          lifecycle_connection(f[["browser"]], "a")[["connection_id"]],
          id
        )
        if (revoked) {
          ...
          testthat::expect_false(
            lifecycle_connection(f[["browser"]], "a")[["status"]] %in%
              c("active", "limited")
          )
        } else {
          retention_browser_wait(
