## Browser E2E: fragment response mode against a live Keycloak realm

if (!exists("make_provider", mode = "function")) {
  source(file.path(dirname(sys.frame(1)$ofile %||% "."), "helper-keycloak.R"))
}

.read_fragment_browser_state <- function(drv) {
...
.wait_for_fragment_auth_url <- function(drv, timeout = 15000) {
  drv$wait_for_js(
    "
    (function () {
      var authUrl = document.querySelector('#auth_url');
  ...
  .read_fragment_browser_state(drv)
}

.navigate_fragment_browser_to_url <- function(drv, url) {
  drv$run_js(paste0(
...
.start_fragment_app <- function(
  ...
  stdout <- tempfile("fragment-app-stdout-", fileext = ".log")
  stderr <- tempfile("fragment-app-stderr-", fileext = ".log")
  ...
  process <- callr::r_bg(
    func = function(
      ...
      if (requireNamespace("pkgload", quietly = TRUE)) {
        ...
        stop(
          paste(
            "Could not load shinyOAuth for the fragment background app.",
      ...
      if (!nzchar(redirect_path) || !startsWith(redirect_path, "/")) {
        stop(
          "redirect_path must be a dedicated non-root path starting with '/'",
          call. = FALSE
        )
      }
      ...
      ui <- shinyOAuth::oauth_fragment_ui(
        base_ui,
        id = "auth",
        client = client
      )
...
.wait_for_fragment_app <- function(app_process, app_port, timeout = 20) {
  ...
  while (Sys.time() < deadline) {
    if (!app_process$process$is_alive()) {
      stop(
        paste(
          "Shiny fragment app exited before it was reachable.",
  ...
  stop(
    paste(
      "Timed out waiting for the Shiny fragment app to listen.",
      .read_log_file(app_process$stderr),
      sep = "\n"
    ),
    call. = FALSE
...
.expect_successful_fragment_browser_flow <- function(
  drv,
  validate_auth_url,
  ...
  testthat::expect_false(
    identical(observed$hash, "#_"),
    info = paste0("Fragment blocker leaked into final hash: ", observed$hash)
  )
  testthat::expect_false(
    grepl("#_$", observed$href),
    info = paste0("Fragment blocker leaked in final href: ", observed$href)
  )
...
testthat::test_that("browser fragment login authenticates through oauth_fragment_ui", {
  maybe_skip_keycloak()
  testthat::skip_if_not_installed("shinytest2")
  testthat::skip_if_not_installed("chromote")
  testthat::skip_if_not_installed("callr")

  app_port <- as.integer(Sys.getenv("SHINYOAUTH_E2E_PORT_FRAGMENT", "8100"))
  if (keycloak_browser_port_in_use(app_port)) {
    testthat::skip(paste0(
      "Port ",
      app_port,
      " is already in use; skipping fragment E2E"
    ))
  }

  provider <- make_provider()
  testthat::expect_true(
    "fragment" %in% (provider@response_modes_supported %||% character())
  )
  repo_root <- .repo_root()
  ...
  app_process <- .start_fragment_app(
    repo_root = repo_root,
    app_port = app_port,
    app_url = app_url,
  ...
  .wait_for_fragment_app(app_process, app_port)

  drv <- shinytest2::AppDriver$new(
    app_url,
    name = "keycloak-fragment-e2e",
    load_timeout = 15000,
    wait = FALSE
  )
  on.exit(try(drv$stop(), silent = TRUE), add = TRUE)

  .expect_successful_fragment_browser_flow(
    drv,
...
testthat::test_that("browser fragment login still succeeds when the auth request is pushed with PAR", {
  maybe_skip_keycloak()
  testthat::skip_if_not_installed("shinytest2")
  testthat::skip_if_not_installed("chromote")
  testthat::skip_if_not_installed("callr")
  ...
  app_port <- as.integer(Sys.getenv(
    "SHINYOAUTH_E2E_PORT_FRAGMENT_PAR",
    "3000"
  ))
  if (keycloak_browser_port_in_use(app_port)) {
    testthat::skip(paste0(
      "Port ",
      app_port,
      " is already in use; skipping fragment + PAR E2E"
    ))
  }
  ...
  drv <- shinytest2::AppDriver$new(
    app_url,
    name = "keycloak-fragment-par-e2e",
    load_timeout = 15000,
    wait = FALSE
  )
  on.exit(try(drv$stop(), silent = TRUE), add = TRUE)
