...
testthat::test_that("proactive refresh keeps session alive with short-lived tokens", {
  ...
  "
    ...
    initial_expires_at <- if (length(initial_expires_match) > 0) {
        as.numeric(sub("expires_at:", "", initial_expires_match))
    } else {
        NA_real_
    }
    ...
    final_expires_at <- if (length(final_expires_match) > 0) {
        as.numeric(sub("expires_at:", "", final_expires_match))
    } else {
        NA_real_
    }
    ...
    testthat::expect_true(
        ...
        info = paste0(
            ...
            "Initial: ", initial_expires_at, ", Final: ", final_expires_at, ". ",
    ...
    testthat::expect_true(
        ...
        info = paste0(
            ...
            "Got: ", token_count, ". Refresh count text: ", refresh_count_text
        )
    )
    ...
    message(
        ...
        initial_expires_at, " to ", final_expires_at,
        " (diff: ", round(final_expires_at - initial_expires_at, 2), "s)"
    )
})
