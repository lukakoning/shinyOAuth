...
- `test_integration_module_shiny_browser_fragment.R` — end-to-end `response_mode=fragment` coverage with a real headless browser: confirms the built authorization URL requests `fragment`, routes the live fragment callback through `oauth_fragment_ui()`, and verifies authenticated state plus cleanup of both the transient callback-handle query parameters and the `#_` blocker fragment.
...
| `test_integration_module_shiny_browser_fragment.R` | OIDC `response_mode=fragment` at the real browser boundary | Verifies Keycloak honors `response_mode=fragment`, the live callback succeeds through `oauth_fragment_ui()`, and the final browser URL/title contain neither OAuth callback params, transient `shinyOAuth_form_post*` handle params, nor the `#_` blocker fragment |
| `test_integration_module_shiny_browser_form_post.R` | OIDC `response_mode=form_post` at the real browser boundary | Verifies Keycloak discovery advertises `form_post`, the built authorization URL requests it, the live POST callback succeeds through `oauth_form_post_ui()`, and the transient `shinyOAuth_form_post*` handle parameters are cleaned from the browser URL/title |
