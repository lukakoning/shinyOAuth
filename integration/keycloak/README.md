...
| `test_integration_keycloak_jar_par_confusion.R` | JAR/PAR outer-parameter behavior | PAR proves outer `redirect_uri`, `scope`, `state`, `nonce`, `resource`, and `code_challenge` do not override the pushed request; direct JAR and JAR+PAR also cover outer `client_id` confusion so no code is accepted and the legitimate flow state remains usable; the JAR-alone case records that this Keycloak fixture still reflects conflicting outer `redirect_uri` and `state` values in an `invalid_request` callback |
