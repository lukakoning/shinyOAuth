...
ui <- fluidPage(
  ...
  p(
    paste(
      "This version uses shinyOAuth's normal auto-redirect flow. Claim a",
      "top-level app URL and use it as OAUTH_REDIRECT_URI and in your",
...
server <- function(input, output, session) {
  ...
  output$login_controls <- renderUI({
    ...
    div(
      ...
      role = "alert",
      paste(
        "Unauthenticated users are redirected to GitHub automatically when",
        "this app is opened at the claimed custom share URL."
      )
    )
  })
