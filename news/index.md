...
- Added JWT Secured Authorization Response Mode (JARM) support for
  ...
  - [`oauth_client()`](https://lukakoning.github.io/shinyOAuth/reference/oauth_client.md)
    now validates signed and signed-then-encrypted
  ...
  - Query JARM validates the compact `response` JWT before any `code`,
    `error`, or `state` values are trusted. `form_post.jwt` uses the
    same validation but requires
    [`oauth_form_post_ui()`](https://lukakoning.github.io/shinyOAuth/reference/oauth_form_post_ui.md)
    with
    [`oauth_module_server()`](https://lukakoning.github.io/shinyOAuth/reference/oauth_module_server.md)
    so Shiny can bridge the POST callback back into the normal module
    flow.
  - Current inbound JARM support accepts signing algorithms `HS256`,
    `HS384`, `HS512`, `RS256`, `RS384`, `RS512`, `ES256`, `ES384`,
    `ES512`, and `EdDSA`. Encrypted JARM currently accepts `RSA-OAEP`
    with `A128CBC-HS256`, `A192CBC-HS384`, or `A256CBC-HS512`.
  - [`oauth_provider_oidc_discover()`](https://lukakoning.github.io/shinyOAuth/reference/oauth_provider_oidc_discover.md)
    now records JARM metadata, the usage docs include minimal JARM setup
    examples, and
    [`oauth_provider_keycloak()`](https://lukakoning.github.io/shinyOAuth/reference/oauth_provider_keycloak.md)
    exposes `tolerate_duplicate_top_level_jarm_iss` for the current
    Keycloak duplicate-issuer interoperability quirk.
