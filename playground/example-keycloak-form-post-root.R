...
# This version lands back on the app root URL instead of /callback.
# The local fixture allows http://127.0.0.1:8100/* for the shiny-public client.
...
redirect_uri <- sprintf("http://127.0.0.1:%d", auth_port)
...
base_ui <- fluidPage(
  titlePanel("shinyOAuth + Keycloak (form_post at app root)"),
  p("Uses the local Keycloak fixture with response_mode = 'form_post'."),
  p("This version returns to the same app URL instead of a /callback sub-route."),
