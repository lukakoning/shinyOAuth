...
**Implementation update, 2026-09-10.** P0-P2 are implemented, including RS384, the generic target/reference objects, their R6 documentation, and independent Python cryptographic/conformance tests. P3 is in progress: encrypted storage (P3a), browser/account owner-session helpers (P3b), and the manager UI/server and lifecycle integration (P3c) are implemented; real-browser retention evidence (P3d) remains required. P4-P7 remain planned. A supplementary P0 checkpoint now supplies the [SMART Dev Sandbox components with official Launcher v2](../integration/smart/sandbox.md), executable smoke tests and a dedicated CI workflow. The upgraded launcher passes 28 smoke assertions, including required discovery fields and public JWKS. The [Inferno STU2.2 Client gate](../integration/smart/inferno.md) is planned for P4/P5 and has not run yet. This establishes the external environment; it does not mark SMART application flows or retention complete. The proposal's baseline comparisons and later interface sketches below remain historical/design context.
...
P3 is split into reviewable implementation items: **P3a** encrypted credential schema and atomic memory store; **P3b** owner sessions and generation/expiry enforcement; **P3c** manager UI/server, callback commit, restoration and lifecycle integration; **P3d** real-browser two-site retention evidence. P3a-P3c are implemented; P3d remains required. P3c has separate commits for managed-module hooks and the public manager API. The [retention checkpoint notes](../integration/smart/retention.md) describe the store, owner and manager contracts and the outstanding browser gate. P3 remains incomplete until the complete A-to-B browser and owner/lifecycle gates pass; helper tests do not establish retention.
...
| P4 discovery | Add `test-smart-discovery.R` against live SMART Launcher v2 metadata and synthetic negative fixtures. | Full-base discovery and capability/registration checks work. The pinned `smart-launcher-2` image (package 2.0.1) supplies the required grant types and S256 fields missing from the older image; smoke tests cover those fields and public JWKS. P4 adds adapter-level acceptance and strict rejection of missing mandatory fields. |
...
| P5 | Add registered EHR launch routes and `test-browser-ehr-launch.R`, driven by the sandbox launcher. | `iss`/`launch` entry, clean continuation, patient/encounter context, simultaneous launches and mixed callback handling pass at the browser boundary. |
