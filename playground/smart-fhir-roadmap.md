...
**Implementation update, 2026-09-10.** P0-P2 are implemented, including RS384, the generic target/reference objects, their R6 documentation, and independent Python cryptographic/conformance tests. P3 is implemented for a single R process: encrypted storage (P3a), browser/account owner sessions (P3b), manager UI/server and lifecycle integration (P3c), and the generic browser-retention gate (P3d). The browser matrix passes 104 assertions across query/form_post and sync/mirai using the explicit HTTP loopback exception; it does not establish SMART conformance. P4-P7 remain planned. A supplementary P0 checkpoint now supplies the [SMART Dev Sandbox components with official Launcher v2](../integration/smart/sandbox.md), executable smoke tests and a dedicated CI workflow. The upgraded launcher passes 28 smoke assertions, including required discovery fields and public JWKS. The [Inferno STU2.2 Client gate](../integration/smart/inferno.md) is planned for P4/P5 and has not run yet. The sandbox smoke checkpoint establishes the external environment; SMART application interoperability is still a P4/P5 gate. The proposal's baseline comparisons and later interface sketches below remain historical/design context.
