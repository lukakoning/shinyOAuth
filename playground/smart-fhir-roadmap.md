...
**Implementation update, 2026-09-10.** P0-P2 are implemented, including RS384, the generic target/reference objects, their R6 documentation, and independent Python cryptographic/conformance tests. P3 is implemented for a single R process: encrypted storage (P3a), browser/account owner sessions (P3b), manager UI/server and lifecycle integration (P3c), and the generic browser-retention gate (P3d). The browser matrix passes 104 assertions across query/form_post and sync/mirai using the explicit HTTP loopback exception; it does not establish SMART conformance. P4 has started with **P4a discovery**: `smart_discover()`, roxygen2 documentation, metadata/endpoint policy tests, and live Launcher v2 discovery integration. It returns a plain metadata snapshot, without creating another R6 object or selecting a client registration. P4b's scope evaluator and login/refresh/introspection/connection integration are implemented; public SMART policy selection arrives with P4c. P4c-P4f and P5-P7 remain planned. The [SMART Dev Sandbox components with official Launcher v2](../integration/smart/sandbox.md) provide executable smoke/discovery tests and a dedicated CI workflow. The [Inferno STU2.2 Client gate](../integration/smart/inferno.md) is planned for P4/P5 and has not run yet. SMART application interoperability is still a P4/P5 gate. The proposal's baseline comparisons and later interface sketches below remain historical/design context.
...
| P4b | SMART semantic scope evaluator, required/optional grants and explicit returned-scope evidence | Implemented; [scope policy and limits](smart-scopes.md). Public selection depends on P4c. |
| P4c | Standalone `smart_target()`, registration/capability selection, S256 and `aud`, identity/signing/freshness policies, supported transport combinations | Planned |
| P4d | Interpreted launch context across refresh, Patient and validated `fhirUser` helpers, context-change lifecycle | Planned |
