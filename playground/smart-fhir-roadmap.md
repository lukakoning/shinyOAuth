...
| --- | --- | --- | --- | --- |
| P0: Baseline and contracts | Shared + SMART: generic multi-server baseline plus SMART fixtures | Document current multiple-module pattern and redirect limitation; commit synthetic interop fixtures and compatibility expectations. | `vignettes/usage.Rmd`, new `tests/testthat/test-smart-contracts.R`, `integration/smart/` | Existing behavior characterized; no claimed persistence from same-session tests. |
