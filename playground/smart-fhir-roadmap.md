...
| P4: SMART standalone | SMART/FHIR adapter over P1-P3 | Add discovery, SMART target factory, semantic scope evaluation, typed context and Patient/user helpers, including SMART-specific signing/freshness policies and transport-combination checks. | New `R/smart_discovery.R`, `R/smart_target.R`, `R/smart_context.R`, `R/smart_scopes.R`, `R/methods__smart_resource.R`; `vignettes/smart-fhir.Rmd` | Standalone patient and clinician apps work, including clinician identity distinct from patient and refresh without repeated context. Multi-site standalone release candidate. |
| P5: SMART EHR launch | SMART/FHIR; reuses generic transaction infrastructure | Add explicit launch routes, approved-target resolution, sealed launch transaction, clean continuation, per-flow parameters and fresh-launch reauthorization behavior. | New `R/smart_launch.R`; targeted changes to the new manager wrapper/router | Normal `iss`/`launch` entry succeeds; ordinary and mixed callbacks retain existing validation; concurrent launches remain separate. Top-level EHR release candidate. |
...
| P4 discovery | Add `test-smart-discovery.R` against the observed sandbox metadata and a conforming SMART 2.2 reference profile. | Full-base discovery and capability/registration checks work. The current launcher 2.0.0 omits required `grant_types_supported` and `code_challenge_methods_supported`; preserve strict rejection tests and upgrade the fixture or select another conforming reference for positive SMART 2.2 evidence. |
