...
and unmodified external release gates remain distinct from fixture and Inferno evidence.
...
**P5a EHR entry is implemented** with `smart_launch_route()`, encrypted owner-bound handoffs, clean continuation, one-use per-flow launch parameters, and a real-browser two-site fixture/CI job. See [setup, tests and supported deployment](../integration/smart/ehr-launch.md). It requires top-level navigation, browser retention and one R process. P5b retains unmodified external EHR compatibility as a release gate. P4c2 transport composition, broader P4d context policy, the remaining P4e vignette/vendor work and P7c-P7e remain planned. P4f and the Inferno part of P5b are implemented as described below.
...
The [official SMART sandbox](../integration/smart/sandbox.md) supplies executable smoke/discovery diagnostics. The [Inferno STU2.2 Client gate](../integration/smart/inferno.md) now has a pinned deployment, real app, browser driver and CI job. It records two explicit simulator corrections while preserving upstream verification tests. The complete command requires 32 two-site scenarios covering public/Basic/RS384/ES384, standalone/EHR, GET/POST and sync/mirai, with retained scope limits and actual post-refresh reads. Unmodified external/vendor interoperability remains unverified; [Oracle registration possibilities](../integration/smart/oracle.md) are documented for later. The proposal's baseline comparisons and later interface sketches below remain historical/design context.
...
| Refresh scope narrowing | General OAuth/OIDC | Managed `$refresh(scopes = ...)`. | Existing calls omit request scope until narrowing succeeds on that connection; later calls retain its accepted limit. |
...
| P4e | Runnable app/vignette, standalone sandbox browser scenarios, two-site SMART retention repeat | Strict fixture and Inferno apps implement the retained-grant scenarios. Vignette and unmodified external sandbox/vendor repeat remain planned or blocked. |
| P4f | Pinned Inferno deployment and real-app public/symmetric/RS384/ES384 matrix, CI and sanitized conformance evidence | Implemented with documented simulator corrections; see the [executed scope and limits](../integration/smart/inferno.md). No unmodified upstream or certification pass is claimed. |
...
| P5b | External EHR scenarios and complete supported registration matrix | Local fixtures and the corrected Inferno simulator cover public, Basic, RS384 and ES384 with actual EHR launch and request verification. Unmodified vendor qualification and Inferno form-post callbacks remain open. Account/session-only EHR entry and optional compositions need separate implementation/tests before support is claimed. |
...
The initial sandbox uses loopback HTTP and synthetic Synthea records. Its FHIR proxy simulates authorization and accepts uncredentialed reads, so data retrieval alone is connectivity evidence. Keep the independent RS384/strict-AS conformance suite and Keycloak tests alongside it; do not treat simulator success as certification or server-side authorization enforcement. Preserve original metadata for the unmodified-sandbox gate. A locally corrected simulator must be separately identified, with its patch digest and unchanged upstream verifier recorded, as in the Inferno gate. Keep signature and claim validation enabled; deployment-specific policies must be explicit. Any legacy profile must be explicit and separately tested. Record transport, registration, capabilities, image IDs, browser versions, and results without credentials or patient context in artifacts.
