...
> **Posit Connect / Connect Cloud:** Open the standalone app's direct URL in a
> new browser tab or window. The dashboard's embedded preview can prevent OAuth
> redirects. On Connect Cloud, copy the sharing URL from **Settings > URL**.
> Keep `/callback/fhir` in the registered `redirect_uri` for this example, but
> open the app's entry URL to start login. For EHR launch, start from the EHR
> using top-level browser navigation and the required launch parameters.
> See [deployment instructions](usage.html#posit-connect-and-connect-cloud).
