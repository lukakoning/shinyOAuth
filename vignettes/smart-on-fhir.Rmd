...
## FHIR, SMART and OAuth in plain terms
...
as the selected patient. An electronic health record (EHR) can open the app, or
the person can open it independently. See [SMART App Launch](https://hl7.org/fhir/smart-app-launch/STU2.2/app-launch.html).

The ordinary code exchange and authenticated HTTP requests remain familiar:

| Step | shinyOAuth interface |
| --- | --- |
| Read a configured server's SMART metadata | `smart_discover()` |
| Combine it with your registered app's settings | `smart_client()`, returning an ordinary `OAuthClient` |
| Authorize and keep separate grants | `oauth_connections_ui()` and `oauth_connections_server()` |
| Use one grant's current credentials | `OAuthConnection$request()` |
| Read its accepted patient or user context | `smart_context()` |
| Fetch the contextual Patient resource | `smart_patient()` |

The manager and connection are package conveniences. SMART does not require
those R objects. They are the same interfaces used for
[multiple ordinary OAuth authorizations](multiple-authorizations.html).
...
Fetching that resource with `smart_fhir_user(connection)` also needs the matching
user-level read permission, for example `user/Practitioner.r` for a Practitioner
reference. Add the appropriate scope to the client registration and requested
scopes; identity scopes alone do not grant permission to read that resource.
