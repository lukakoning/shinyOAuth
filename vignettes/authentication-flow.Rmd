...
On first page load, the module asks the browser to create a random token in a
cookie and pass its value to Shiny as an input. This lets shinyOAuth check later
...
The cookie is readable by JavaScript so its value can reach Shiny. The browser
must support cookies and Web Crypto; if the cookie cannot be created or read,
the module reports `browser_cookie_error` and stops login. An invalid browser
token is rejected and replaced before login or callback processing continues.

Wrap your UI in
[`oauth_ui()`](https://lukakoning.github.io/shinyOAuth/reference/oauth_ui.html)
to prevent callback addresses from being sent as referrers when the page loads.
