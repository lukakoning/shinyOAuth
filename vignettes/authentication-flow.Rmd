...
the same sender-constrained path. Treat that preserved thumbprint as continuity
state rather than fresh proof of binding for the new token; enable
`oauth_client(introspect = TRUE)` if you need stronger assurance for opaque
refresh responses.
...
shinyOAuth may preserve a prior certificate thumbprint so mTLS-bound resource
requests do not silently lose their routing state. This is not fresh proof that
the new access token is still sender-constrained; use introspection when that
assurance matters.
