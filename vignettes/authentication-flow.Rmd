...
shinyOAuth does not automatically replay a `DPoP-Nonce` challenge because those
requests may consume a single-use credential. Instead it raises a typed error so
the caller can decide whether replay is safe. The response must include at least
`access_token`.
...
`tls_client_certificate_bound_access_tokens = TRUE`, shinyOAuth prefers the
mTLS endpoints when available and treats the resulting tokens as
certificate-bound:
...
- Authorization-code exchange and refresh calls to the token endpoint carry a DPoP proof.
  ...
  surfaces a typed nonce-challenge error instead of replaying the same
  non-idempotent request automatically
