...
- If the callback is an error response (`?error=...`), require a valid `state` parameter and defer
  ...
  rather than surfacing the attacker-controlled `?error` value. The `error_uri` from the provider
  (RFC 6749 section 4.1.2.1) is also surfaced as a reactive field when included
