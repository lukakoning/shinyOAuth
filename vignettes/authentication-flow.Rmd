...
- The response must include a new `access_token`. `expires_at` is updated from `expires_in` when present.
...
`oauth_module_server(indefinite_session = TRUE)`, the token is kept but marked stale. In all cases, the
