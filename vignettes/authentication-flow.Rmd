...
Configured authorization endpoint queries are preserved. Matching managed
singleton values are emitted once; conflicting or repeated singleton values
are rejected before use. Repeated `resource` indicators remain supported, and
parameter names are case sensitive. The same composition checks apply to
direct, PAR and JAR browser URLs.
...
limits still apply; oversized values are rejected without truncation. Tests
cover an encoded 8000-byte request-line envelope, but larger codes, state or
JARM values can exceed a deployment's proxy limits. Check the whole encoded
callback when configuring those limits.
...
Encrypted JARM requires distinct callback routes, even with an outer `iss`;
shared encrypted routes are rejected at configuration time. Issuer routing never replaces signature,
