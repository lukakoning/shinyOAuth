...
- Later `get_userinfo()` calls and downstream `perform_client_bearer_req()` requests also
  attach DPoP proofs when the effective access-token type is `DPoP` and the caller supplies
  the corresponding `OAuthClient`
  - `perform_client_bearer_req()` also retries one protected-resource
    `use_dpop_nonce` challenge with the supplied `DPoP-Nonce`; plain
    `client_bearer_req()` only builds the request object
