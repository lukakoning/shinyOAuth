...
If JAR is also enabled, the Request Object pushed to the PAR endpoint follows
the same rule: signed by default, or signed first and then encrypted when
Request Object encryption is configured. The browser still sees only
`client_id` plus `request_uri`.
