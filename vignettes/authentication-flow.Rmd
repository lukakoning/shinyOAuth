...
style: HTTP Basic, body client secret, `client_secret_jwt`, or `private_key_jwt`.
...
- If the callback includes an `iss` query parameter, validate it against the provider's configured/discovered
  issuer to defend against authorization-server mix-up attacks (per RFC 9207). A mismatch produces an
  `issuer_mismatch` error and audit event
