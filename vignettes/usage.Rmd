...
example inside `fluidPage()`, `tagList()`, or `bslib::page()`. If your app UI
is wrapped with `oauth_form_post_ui()` or `oauth_fragment_ui()`, those wrappers
inject the same dependency automatically.
...
## Using `response_mode = "fragment"`

Some providers also allow or require `response_mode = "fragment"` for the
authorization-code flow. In that mode the provider redirects back with
parameters such as `code`, `state`, `error`, and `iss` in the URL fragment
(for example, `#code=...&state=...`). Because browsers do not send fragments to
the server on the first request, server-rendered Shiny apps need a small
browser bridge before `oauth_module_server()` can resume the normal callback
flow.

shinyOAuth supports the plain fragment response mode through
`oauth_fragment_ui()`. That wrapper serves a minimal callback page on a
dedicated non-root path, reads the fragment in the browser, POSTs it back to
the same path, and then reuses the existing one-time callback-handle flow.
The follow-up redirect also applies a blocker fragment so the original OAuth
fragment is not inherited by later redirects.

Use fragment mode only when your provider explicitly requires or recommends it.
For server-side Shiny apps, query remains the default and `form_post` is often
the better explicit alternative when the provider supports it.

This is the plain OAuth/OIDC fragment response mode. JWT Secured Authorization
Response Mode (JARM) values such as `fragment.jwt` use a different JWT
`response` payload and are not currently supported.
...
client <- oauth_client(
  ...
  client_id = "shiny-public",
  client_secret = "",
  # Fragment mode requires a dedicated non-root callback path.
  redirect_uri = "http://127.0.0.1:8100/callback",
  scopes = c("openid", "profile", "email"),
...
Because the callback path is a sub-route such as `/callback`, use
`uiPattern = ".*"` so Shiny routes both the initial callback GET and the
follow-up POST through `oauth_fragment_ui()` before the app returns to its
normal GET flow.
