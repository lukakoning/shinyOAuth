...
`oauth_provider_oidc_discover()` fills those provider fields automatically. If
you configure the provider manually, add the advertised JARM metadata to the
provider object and tell the client which signed or encrypted callback to
expect. A minimal signed-JARM setup looks like this:
