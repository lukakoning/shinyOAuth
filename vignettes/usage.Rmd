...
<!-- Callback referrer protection -->

Wrap the complete UI with `oauth_ui(ui)` to send `Referrer-Policy: no-referrer`
before callback-page dependencies load. This wrapper also includes `use_shinyOAuth()`.
Alternatively, set that header at the web server. The meta tag from
`use_shinyOAuth()` alone arrives after Shiny dependencies and cannot protect their
initial requests. `oauth_form_post_ui()` applies the HTTP header automatically.
