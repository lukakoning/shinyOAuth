...
wraps them in a signed JWT, and sends them via the browser redirect's
`request` parameter instead of placing raw OAuth parameters on the URL.
...
- If a provider also exposes PAR, `authorization_request_mode = "request"`
  takes precedence and keeps the browser redirect on the direct `request`
  path.
