...
For writes, use `configure` to add a body or application headers to the
unauthenticated request. Supply the method and required permissions explicitly:

```{r multiple-request-body, eval = FALSE}
connection$request("api", "records/example", method = "PUT",
...
The function may change only the body and application headers. The connection
...
accept `max_entries` (default 1,000) per manager. Browser entries include recent
visitors who never authorize a service. Account entries include logged-out login
generations until their reauthentication deadline. A full registry rejects new
owners while preserving existing sessions and retirement records.
