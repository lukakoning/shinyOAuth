...
## What this adds to an ordinary OAuth app

Suppose someone connects Service A, then connects Service B, and wants to keep
using both. Each authorization gives the app its own credentials. Starting the
second authorization navigates away from Shiny and can end the session holding
the first credentials.

The connection manager keeps those authorizations separately and can restore
them when the browser returns. You still configure ordinary OAuth clients and
use the package's request helpers. The extra pieces handle selection, storage,
ownership, refresh and disconnecting.
...
These are package interfaces, usable with ordinary OAuth as well as SMART.
Neither OAuth nor SMART requires these particular R objects. One client can
have several connections: connecting A again creates another authorization;
it does not replace the earlier one or guarantee that a different external
account was selected.

For a single authorization that only needs to last for one Shiny session,
the existing `oauth_ui()` and `oauth_module_server()` pattern in
[Usage](usage.html) is still sufficient.
