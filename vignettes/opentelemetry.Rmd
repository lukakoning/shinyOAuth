...
- When: when a resumed form-post callback must consume its sealed state from
  the state store because the pre-session bridge did not supply stored state
  values
- Represents: single-use state lookup and removal before callback validation
- Main attributes:
  - `oauth.provider.name`, `oauth.provider.issuer`
  - `oauth.client_id_digest`
  - `shiny.module_id`
