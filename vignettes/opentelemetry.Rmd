...
For shinyOAuth-managed async work, the package restores the parent session's
captured `OTEL_*` / `OTEL_R_*` environment variables inside mirai workers before
running OAuth logic. This keeps exporter selection and OTLP endpoints
consistent across the main process and worker processes.
