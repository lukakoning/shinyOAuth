...
- `mirai_error_type`: a top-level event field on certain async failure events. It classifies mirai transport-level failures separately from application-level errors. Present on `login_failed`, `session_cleared`, and `refresh_failed_but_kept_session` events:
...
refresh, and revocation run through shinyOAuth's async dispatch layer. With
...
there as well. For shinyOAuth-managed async work, the package also replays the
