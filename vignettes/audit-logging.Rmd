...
Audit and trace events emitted via OpenTelemetry mirror the events described in this 
vignette.
...
This means you can safely forward the `shiny_session$http` object to external logging systems without manually stripping secrets.
...
automatically propagates your `shinyOAuth.audit_hook` and `shinyOAuth.trace_hook`
options to these workers, so audit events fire also in the async worker processes and
