...
default_client_label <- function(provider) {
  ...
  if (!is_valid_string(provider@name)) {
  ...
  tryCatch(
    {
      ...
      if (
        length(client@required_scopes) &&
          !identical(
            client_scope_coverage(
              client,
              client@required_scopes,
              effective_client_scopes(client, warn = FALSE)
            )[["status"]],
            "covered"
          )
      ) {
        return(
          "OAuthClient: Required scopes must be included in the client's requested scopes"
        )
      }
      if (
        !is_valid_string(client@label) ||
          nchar(client@label, type = "bytes") > 128L ||
          grepl("[[:cntrl:]]", client@label)
      ) {
        return(
          "OAuthClient: label must be a non-empty string of at most 128 bytes"
        )
      }
      NULL
    },
