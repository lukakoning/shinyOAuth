...
#' Warn about unenforced claim requests
#'
#' Warns when the caller requests enforceable OIDC claims but leaves claim
#' validation at the default `"none"` mode. Used by [oauth_client()] after the
#' claims-related inputs have been normalized.
#'
#' @param claims Claims request passed to [oauth_client()].
#' @param claims_validation Claims validation mode selected by the caller.
#' @param claims_validation_missing Whether the caller left
#'   `claims_validation` at its default.
#' @return Invisibly returns `TRUE` when a warning is emitted; otherwise
