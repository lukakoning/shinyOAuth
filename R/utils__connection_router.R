...
connection_router_key <- function(manager) {
  as.raw(openssl::sha256(
    charToRaw(paste0(
      "shinyOAuth/connection-router/v1/",
      manager[["app_origin"]],
      "/",
      manager[["state"]][["id"]]
    )),
...
connection_router_digest <- function(state) {
  if (
    !is_valid_string(state) ||
      nchar(state, type = "bytes") > oauth_callback_limits()[["state"]]
  ) {
...
connection_router_register <- function(
  manager,
  client_name,
  context,
  prepared
  ...
  context_digest <- authorization_context_digest(authorization_context_json(
    context
  ))
  if (
    is.null(pending) ||
      !is.null(pending[["route_digest"]]) ||
      !identical(
        context_digest,
        authorization_context_digest(authorization_context_json(pending[[
          "context"
        ]]))
      ) ||
  ...
  if (
    !identical(payload[["transaction_context_digest"]], context_digest) ||
      !identical(
        prepared[["state_key"]],
        state_cache_key(payload[["state"]])
      ) ||
      !is.null(manager[["state"]][["routes"]][[digest]]) ||
      length(manager[["state"]][["routes"]]) >= 1000L
  ) {
...
connection_router_select <- function(manager, candidates, payload) {
  ...
  if (
    !identical(route[["purpose"]], "connection-router-v1") ||
      !identical(route[["manager"]], manager[["state"]][["id"]]) ||
      !identical(route[["digest"]], digest) ||
      !is.numeric(route[["expires_at"]]) ||
      length(route[["expires_at"]]) != 1L ||
      !is.finite(route[["expires_at"]]) ||
      route[["expires_at"]] <= as.numeric(Sys.time()) ||
      is.null(pending) ||
      is.null(client) ||
      !identical(pending[["route_digest"]], digest) ||
      !identical(pending[["context"]][["client"]], route[["client"]]) ||
      !identical(
        route[["context_digest"]],
        authorization_context_digest(authorization_context_json(pending[[
          "context"
        ]]))
      ) ||
  ...
  if (!id %in% names(candidates)) {
    err_invalid_state("Callback client does not match its route")
  }
