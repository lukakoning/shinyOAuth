test_that("revocation requires 200 and never claims another status revoked a token", {
  ...
  for (status in c(200L, 201L, 202L, 204L, 299L)) {
    ...
    if (status == 200L) expect_true(result[["revoked"]]) else {
...
test_that("UserInfo and introspection reject JSON under unrelated media types", {
  ...
  token <- OAuthToken(access_token = "opaque")
  for (type in c("text/plain", "text/html", "application/octet-stream", "application/jwtx")) {
    response <- httr2::response(status = 200, headers = list("content-type" = type),
  ...
  for (type in c("application/json", "application/json; charset=utf-8", "application/vnd.github+json")) {
    expect_true(response_has_json_media_type(httr2::response(status = 200,
      headers = list("content-type" = type), body = raw())))
  }
})
