test_that("SMART extension URLs never become credential destinations", {
  ...
  requests <- list()
  result <- revoke_token(client, token, which = "refresh", async = FALSE)
  ...
  site[["metadata"]][["revocation_endpoint"]] <- "https://unapproved.example/revoke"
  expect_error(smart_client(site, "example", "https://app.example/callback",
    scopes = "user/Patient.r"), "outside endpoint_hosts",
  ...
  client <- smart_client(site, "example", "https://app.example/callback",
    ...
    client_secret = "example-client-secret")
  requests <- list()
  expect_true(revoke_token(client, token, which = "refresh", async = FALSE)[["revoked"]])
  expect_length(requests, 1L)
  expect_identical(requests[[1L]][["url"]], "https://ehr.example/revoke")
  expect_identical(as.character(requests[[1L]][["body"]][["data"]][["token"]]), "example-refresh")
  authorization <- requests[[1L]][["headers"]][["Authorization"]]
  if (typeof(authorization) == "weakref") authorization <- rlang::wref_value(authorization)
  expect_identical(authorization,
    paste0("Basic ", openssl::base64_encode(charToRaw("example:example-client-secret"))))
})

test_that("SMART policy extensions do not enable protocol requirements", {
  for (field in c("require_pushed_authorization_requests", "require_signed_request_object",
    ...
    if (field == "authorization_response_iss_parameter_supported") {
      exact <- smart_client(site, "example", "https://app.example/callback", scopes = "user/Patient.r")
      expect_true(exact@provider@authorization_response_iss_parameter_supported)
    } else {
      expect_error(smart_client(site, "example", "https://app.example/callback",
        scopes = "user/Patient.r"), "required PAR/JAR")
    }
  }
})
...
test_that("SMART snapshots and required metadata require exact member names", {
  ...
  for (field in c("smart_version", "metadata", "allow_http_loopback", "fhir_base", "endpoint_hosts")) {
    altered <- site
    names(altered)[names(altered) == field] <- paste0(field, "Alias")
    expect_error(smart_client(altered, "example", "https://app.example/callback",
      scopes = "user/Patient.r"), class = if (field == "allow_http_loopback") {
        "shinyOAuth_input_error"
      } else "shinyOAuth_config_error")
  }
  for (field in c("authorization_endpoint", "token_endpoint", "issuer", "jwks_uri",
    ...
    altered <- site
    names(altered[["metadata"]])[names(altered[["metadata"]]) == field] <- paste0(field, "Alias")
    expect_error(smart_client(altered, "example", "https://app.example/callback",
      scopes = "user/Patient.r", identity = "fhirUser"), class = "shinyOAuth_parse_error")
  }
})
...
test_that("SMART token checks require exact type and lifetime members", {
  client <- smart_client(smart_client_fixture(), "example", "https://app.example/callback",
    scopes = "user/Patient.r")
  response <- list(access_token = "example-access", token_type = "Bearer",
    expires_in = 300, scope = "user/Patient.r")
  for (refresh in c(FALSE, TRUE)) {
    for (field in c("token_type", "expires_in")) {
      altered <- response
      names(altered)[names(altered) == field] <- paste0(field, "Alias")
      expect_error(verify_token_set(client, altered, nonce = NULL, is_refresh = refresh,
        prior_granted_scopes = "user/Patient.r"),
        field, class = "shinyOAuth_token_error")
...
test_that("SMART context and launch parameters do not accept similarly named extensions", {
  ...
  token <- OAuthToken(access_token = "example-access", token_type = "Bearer",
    granted_scopes = "patient/Patient.r", granted_scopes_verified = TRUE,
    extra_fields = list(patientAlias = "example-patient", encounterAlias = "example-encounter",
      need_patient_bannerAlias = TRUE))
  expect_error(smart_update_token_context(client, token), "require patient context")
  ...
  for (query in c("issAlias=https://ehr.example/fhir/R4&launch=example",
