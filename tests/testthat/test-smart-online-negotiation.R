...
test_that("explicit refresh narrowing retains negotiated offline access on the wire", {
  ...
  accepted <- verify_token_set(client, list(access_token = "access", token_type = "Bearer",
    expires_in = 300, scope = "user/Patient.rs offline_access"), NULL)
  ...
  expect_no_error(validate_refresh_scope_request(prepare_client_for_worker(client), token, request))
  seen <- NULL
  local_mocked_bindings(req_with_retry = function(req, ...) {
    ...
    httr2::response(url = req[["url"]], status = 200,
      ...
      body = charToRaw(jsonlite::toJSON(list(access_token = "new-access",
        ...
        scope = paste(scopes, collapse = " ")), auto_unbox = TRUE)))
  }, .package = "shinyOAuth")
  ...
  expect_setequal(strsplit(utils::URLdecode(as.character(seen[["scope"]])), " ")[[1L]], scopes)
  expect_setequal(refreshed@granted_scopes, scopes)
  expect_identical(refreshed@refresh_token, "rotated")
  expect_identical(refreshed@original_granted_scopes, token@original_granted_scopes)
  expect_setequal(effective_client_scopes(client), c("user/Patient.rs", "online_access", "launch"))
  expect_error(refresh_scope_request(client, refreshed, c("user/Patient.rs", "offline_access")), "covered")
  expect_error(refresh_scope_request(client, refreshed, "user/Patient.r"), "required")
})

test_that("negotiated refresh ceilings require opt-in and retain other scope limits", {
  scopes <- c("user/Patient.r", "offline_access")
  token <- OAuthToken(access_token = "access", granted_scopes = scopes, granted_scopes_verified = TRUE)
  expect_error(refresh_scope_request(online_negotiation_client(), token, scopes), "configuration")
  client <- online_negotiation_client("allow_offline")
  online <- token
  online@granted_scopes <- c("user/Patient.r", "online_access")
  expect_error(refresh_scope_request(client, online, scopes), "covered")
  unrequested <- online_negotiation_client("allow_offline", required = character(), scopes = "user/Patient.r")
  expect_error(refresh_scope_request(unrequested, token, scopes), "configuration")
  token@granted_scopes <- c("user/Patient.rs", "offline_access")
  expect_error(refresh_scope_request(client, token, token@granted_scopes), "configuration")
  explicit <- online_negotiation_client(required = "offline_access", scopes = scopes)
  expect_setequal(refresh_scope_request(explicit, token, scopes)[["scopes"]], scopes)
})
