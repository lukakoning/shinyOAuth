...
test_that("runtime metadata retrieval obeys tightened host and scheme policy", {
  for (issuer in c("https://retired.example", "http://retired.example")) {
    local_options(
      ...
      shinyOAuth.allowed_non_https_hosts = c("retired.example", "localhost", "127.0.0.1")
    )
    ...
    local_mocked_bindings(req_with_retry = function(req, ...) {
      ...
      body <- if (url == client@provider@userinfo_url) {
        ...
        as.character(jsonlite::toJSON(
          if (url == "https://example.com/jwks") fixture[["jwks"]] else
      ...
      httr2::response(
        url = url,
        status_code = 200L,
        headers = list(`content-type` = if (url == client@provider@userinfo_url)
          "application/jwt" else "application/json"),
        body = charToRaw(body)
      )
    })
    ...
    expect_identical(get_userinfo(explicit_client, "synthetic-access")[["sub"]], "subject")
