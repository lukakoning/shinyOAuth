router_controller <- function(manager, cookie) {
  ...
  controller <- shiny::withReactiveDomain(
    session,
    connection_manager_controller(manager, session)
  )
  # Give direct fixture calls the same owning reactive domain as a Shiny module.
  wrap <- function(value) {
    if (is.list(value)) {
      return(lapply(value, wrap))
    }
    if (!is.function(value)) {
      return(value)
    }
    function(...) {
      shiny::withReactiveDomain(session, shiny::isolate(wrap(value(...))))
    }
  }
  wrap(controller)
}

router_fixture <- function(
  post = FALSE,
  jarm = FALSE,
  policy = "shared_routes",
  distinct_issuers = FALSE,
  par = FALSE,
  ...
  clients <- lapply(c("a", "b"), function(id) {
    ...
    client <- oauth_client(
      provider,
      client_id = id,
      client_secret = strrep(id, 64),
      redirect_uri = paste0(
        "https://app.example/callback",
        if (distinct) paste0("/", id)
      ),
      state_key = strrep(id, 64),
      scopes = "read",
      authorization_server_mode = if (distinct) {
        "multi_redirect_uri"
      } else {
        "multi_issuer"
      },
      authorization_server_redirect_uris = if (distinct) {
        paste0("https://app.example/callback/", c("a", "b"))
      } else {
        character()
      },
  ...
  if (same_registration) {
    clients[["b"]] <- connection_test_client(
      clients[["a"]],
      c(api = "https://api.example/b"),
      "read",
      "b"
    )
  }
  ...
  ui <- oauth_connections_ui(
    shiny::fluidPage("Shared callback"),
    "health",
    manager
  )
  ...
  f[["clients"]] <- stats::setNames(
    lapply(clients, function(client) client),
    c("health-a", "health-b")
  )
  f
}
...
router_prepare <- function(f, id = "a", browser = valid_browser_token()) {
  ...
  if (is.function(hooks[["prepared"]])) {
    hooks[["prepared"]](prepared, context)
  }
  list(
    context = context,
    prepared = prepared,
...
router_request <- function(
  ...
  fields <- c(
    list(state = auth[["state"]], iss = client@provider@issuer),
    ...
    fields <- list(
      response = jose::jwt_encode_hmac(
        do.call(
          ...
          c(
            fields,
            list(aud = client@client_id, exp = as.numeric(Sys.time()) + 60)
          )
        ),
...
test_that("shared issuer routing is explicit and leaves the legacy registry strict", {
  f <- router_fixture()
  expect_error(
    oauth_connections(f[["manager"]][["clients"]], "https://app.example"),
    "multi_redirect_uri"
  )
  ...
  expect_error(
    oauth_ui(shiny::fluidPage(), clients = f[["clients"]]),
    "distinct issuers"
  )
  expect_error(
    oauth_connections(
      list(
        a = connection_test_client(
          make_test_client(),
  ...
  for (id in c("a", "b")) {
    ...
    expect_identical(
      issuer[["ui"]](router_request(issuer, auth))[["status"]],
      303L
    )
  }
})
...
for (post in c(FALSE, TRUE)) {
  for (jarm in c(FALSE, TRUE)) {
    for (failure in c(FALSE, TRUE)) {
      test_that(
        paste(
          "same-issuer bridge selects original registration",
          post,
          jarm,
          failure
        ),
        {
          ...
          authorizations <- lapply(c("a", "b"), function(id) {
            router_prepare(f, id)
          })
          ...
          for (id in c("b", "a")) {
            ...
            expect_null(f[["manager"]][["state"]][["pending"]][[auth[[
              "context"
            ]][["transaction"]]]])
          }
          ...
          if (!failure) {
            expect_setequal(
              vapply(
                rows,
                function(row) row[["stored"]][["client"]],
                character(1)
              ),
              c("a", "b")
            )
...
test_that("routing cannot replace issuer, state, signature or registration verification", {
  for (jarm in c(FALSE, TRUE)) {
    ...
    logical_state <- state_payload_decrypt_validate(
      f[["manager"]][["clients"]][["a"]],
      auth[["state"]]
    )[["state"]]
    ...
    bad <- list(
      router_request(
        ...
        auth,
        jarm = jarm,
        overrides = list(iss = "https://wrong.example")
      ),
      ...
      router_request(
        ...
        jarm = jarm,
        overrides = if (jarm) {
          list(response = "invalid.jwt.value")
        } else {
          list(state = "unknown")
        }
      )
...
test_that("routing records bind manager, client, exact state and live pending context", {
  ...
  for (field in c(
    ...
    manager_state[["routes"]][[key]] <- state_encrypt_gcm(
      changed,
      connection_router_key(f[["manager"]])
    )
    expect_error(select())
  }
...
test_that("expired routing entries are rejected read-only and pruned on preparation", {
  f <- router_fixture()
  auth <- router_prepare(f)
  pending <- f[["manager"]][["state"]][["pending"]][[auth[["context"]][[
    "transaction"
  ]]]]
  ...
  expect_length(f[["manager"]][["state"]][["routes"]], 0L)
  expect_null(f[["manager"]][["state"]][["pending"]][[auth[["context"]][[
    "transaction"
  ]]]])
})
...
for (post in c(FALSE, TRUE)) {
  for (failure in c(FALSE, TRUE)) {
    test_that(
      paste(
        "distinct routes accept encrypted JARM without outer issuer",
        post,
        failure
      ),
      {
        ...
        for (id in c("b", "a")) {
          ...
          encoded <- if (post) {
            rawToChar(request[["rook.input"]][["read"]]())
          } else {
...
test_that("module registers structured state before provider work and cancels failed preparation", {
  ...
  local_mocked_bindings(build_prepared_authorization = function(
    ...
    called <<- TRUE
    expect_identical(
      names(connection_router_select(
        f[["manager"]],
        f[["clients"]],
        list(state = prepared[["build_args"]][["payload"]])
      )),
      "health-a"
    )
  ...
  local_mocked_bindings(async_dispatch = function(expr, args, ...) {
    ...
    expect_identical(
      names(connection_router_select(
        f[["manager"]],
        f[["clients"]],
        list(state = dispatched[["build_args"]][["payload"]])
      )),
      "health-a"
    )
    promises::promise(function(resolve, reject) {
      reject(simpleError("synthetic PAR failure"))
    })
  })
  shiny::testServer(
    oauth_module_server_impl,
...
test_that("route, transport, parsing bounds and issuer checks precede index lookup", {
  ...
  for (request in list(
    ...
    router_request(
      f,
      auth,
      overrides = list(iss = "https://unrecognized.example")
    ),
    router_request(
      f,
      auth,
      overrides = list(
        state = strrep("x", oauth_callback_limits()[["state"]] + 1L)
      )
    )
  )) {
    expect_identical(f[["ui"]](request)[["status"]], 400L)
...
test_that("selected signed JARM still requires signature, audience and expiry", {
  ...
  for (change in list(
    list(aud = "another-registration"),
    list(exp = 1),
    list(iss = "https://other.example")
  )) {
    response <- jose::jwt_encode_hmac(
      do.call(jose::jwt_claim, utils::modifyList(claims, change)),
      client@client_secret
    )
    expect_identical(
      f[["ui"]](router_request(
        ...
        auth,
        jarm = TRUE,
        overrides = list(response = response)
      ))[["status"]],
      400L
    )
    expect_length(f[["manager"]][["state"]][["routes"]], 1L)
  ...
  expect_identical(
    f[["ui"]](router_request(f, auth, jarm = TRUE))[["status"]],
    303L
  )
})
...
test_that("one registration can retain distinct resource clients on its shared route", {
  ...
  expect_identical(
    f[["manager"]][["clients"]][["a"]]@client_id,
    f[["manager"]][["clients"]][["b"]]@client_id
  )
  expect_identical(
    f[["manager"]][["clients"]][["a"]]@provider,
    f[["manager"]][["clients"]][["b"]]@provider
  )
  expect_false(identical(
    f[["manager"]][["fingerprints"]][["a"]],
    f[["manager"]][["fingerprints"]][["b"]]
  ))
