...
for (store in c("url", "server")) {
  for (scrub_only in c(FALSE, TRUE)) {
    test_that(
      paste(store, "bookmarks omit private module inputs; hook", scrub_only),
      {
        ...
        for (prefix in c("auth-", "outer-auth-")) {
          inputs <- list(
            token,
            list(requestId = "bookmark", token = token),
            token
          )
          names(inputs) <- paste0(
            prefix,
        ...
        app$click("bookmark")
        app$wait_for_js(
          "document.getElementById('bookmark_url').innerText.includes('?')"
        )
        ...
        expect_false(grepl(
          "shinyOAuth_(sid|cookie_ack|cookie_error)",
          serialized
        ))
...
test_that("a disclosed binding cannot select login state or validate a swapped callback", {
  skip_if_not(tolower(Sys.getenv("SHINYOAUTH_BROWSER_TESTS")) == "true")
  app <- shinytest2::AppDriver$new(
    bookmark_binding_app("url", withr::local_tempdir()),
    load_timeout = 15000
  )
  on.exit(stop_test_app_driver(app), add = TRUE)
  ...
  app$wait_for_js(
    "document.getElementById('prepared_url').innerText.includes('state=')"
  )
  url <- app$get_value(output = "prepared_url")
  callback <- paste0(
    parse_query_param(url, "redirect_uri", decode = TRUE),
    "?code=unused-local-fixture&state=",
    parse_query_param(url, "state")
  )
  app$get_chromote_session()$go_to(callback)
  app$wait_for_js(
    "document.getElementById('auth_error')?.innerText === 'invalid_state'"
  )
  # A full navigation replaces shinytest2's injected tracer; read the live DOM.
  expect_identical(
    app$get_js("document.getElementById('binding').innerText"),
    previous
  )
  expect_identical(
    app$get_js("document.getElementById('state_count').innerText"),
    "1"
  )
  expect_identical(
    app$get_js("document.getElementById('authenticated').innerText"),
    "FALSE"
  )
})
