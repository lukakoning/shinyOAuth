...
test_that("authorization POST rejects case variants of the browser charset field", {
  ...
  for (name in c("_charset_", "_CHARSET_", "_cHaRsEt_")) {
    ...
    request <- list(method = "POST", url = "https://provider.example/authorize",
    ...
    expect_identical(result[["result"]][["value"]][["submissions"]], 0L, info = name)
    expect_identical(result[["result"]][["value"]][["remainingForms"]], 0L, info = name)
  }
})
