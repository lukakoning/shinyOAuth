test_that("automatic retry cooldown follows shared credentials across records and owners", {
  ...
  local_mocked_bindings(req_with_retry = function(req, ...) {
    ...
    httr2::response(url = req[["url"]], status = 503L,
  ...
  shiny::testServer(oauth_connections_server, args = args,
    session = manager_test_session(cookie), {
      for (i in 1:3) manager_test_accept(controller, token = token)
      session[["flushReact"]]()
      expect_length(attempts, 1L)
      expect_true(all(vapply(controller[["records"]](), function(row) row[["status"]] == "active", logical(1))))
      ...
      expect_equal(as.numeric(difftime(attempts[[2L]], attempts[[1L]], units = "secs")), 30)
    })
  ...
  shiny::testServer(oauth_connections_server, args = args,
    session = manager_test_session(cookie), {
      ...
      expect_error(session[["getReturned"]]()[["connection"]](id)[["refresh"]](), "Connection refresh failed")
...
test_that("queued automatic refreshes recheck the cooldown after asynchronous failure", {
  ...
  shiny::testServer(session = manager_test_session(manager_test_cookie(f)),
    function(input, output, session) {
      ...
      failed <- resumed <- NULL
      promises::catch(ctl[["refresh"]](a, async = TRUE, touch = FALSE), function(e) failed <<- e)
      promises::then(ctl[["refresh"]](b, async = TRUE, touch = FALSE),
        function(value) resumed <<- value, function(e) resumed <<- e)
      reject_refresh(refresh_outcome_error(simpleError("provider unavailable"), "not_consumed"))
      poll_for_async(function() !is.null(failed) && (!is.null(resumed) || calls > 1L), session)
