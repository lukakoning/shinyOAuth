test_that("SMART property edits retain transport and discovery host policy", {
  ...
  expect_error(client@redirect_uri <- "http://localhost/callback", "requires HTTPS")
  expect_error(client@provider@token_url <- "http://localhost/token", "requires HTTPS")
  for (field in c("auth_url", "token_url", "revocation_url", "introspection_url", "userinfo_url")) {
    provider <- client@provider
    S7::prop(provider, field) <- "https://outside.example/endpoint"
    expect_error(client@provider <- provider, "outside endpoint_hosts")
  }
  expect_no_error(client@provider@token_url <- "https://ehr.example/another-token")
  ...
  dev <- smart_client(site, "example", "http://localhost/callback", scopes = "user/Patient.r")
...
test_that("SMART scope edits retain app launch and advertised capabilities", {
  ...
  client <- smart_client(site, "example", "https://app.example/callback",
    scopes = "user/Patient.r", required_scopes = character(), allow_v1 = TRUE)
  for (scope in c("system/Patient.r", "patient/Patient.r", "launch", "openid", "fhirUser",
      "offline_access", "launch/encounter")) {
  ...
  ehr <- smart_client(site, "example", "https://app.example/callback",
    scopes = "user/Patient.r", required_scopes = character(), launch = "ehr")
  ...
  site[["metadata"]][["capabilities"]] <- as.list(setdiff(
    unlist(site[["metadata"]][["capabilities"]]), "permission-v1"))
  v2 <- smart_client(site, "example", "https://app.example/callback",
    scopes = "user/Patient.r", required_scopes = character(), allow_v1 = TRUE)
...
test_that("SMART authentication edits retain capability and method advertisements", {
  site <- smart_client_fixture()
  site[["metadata"]][["capabilities"]] <- as.list(setdiff(
    unlist(site[["metadata"]][["capabilities"]]), "client-confidential-symmetric"))
  client <- smart_client(site, "example", "https://app.example/callback", scopes = "user/Patient.r")
  provider <- client@provider
  provider@token_auth_style <- "header"
  expect_error(S7::props(client) <- list(provider = provider, client_secret = "example-secret"),
    "client-confidential-symmetric")
  tampered <- client@smart
  tampered[["discovery"]][["endpoint_hosts"]] <- c("ehr.example", "outside.example")
...
test_that("SMART operations revalidate configuration restored without S7 setters", {
  ...
  local_mocked_bindings(req_with_retry = function(...) stop("Unexpected transport"),
    .package = "shinyOAuth")
