...
test_that("SMART v2 covers unions, interaction subsets and resource wildcards", {
  ...
  cases <- list(
    list(
      ...
      c("patient/Observation.r", "patient/Observation.s"),
      "covered"
    ),
    list("patient/Observation.r", "patient/*.rs", "covered"),
    list(
      "patient/*.rs",
      c("patient/Patient.rs", "patient/Observation.rs"),
      "insufficient"
    ),
    list("patient/*.rs", c("patient/*.r", "patient/*.s"), "covered"),
    list(
      "patient/Observation.cruds",
      c("patient/Observation.rs", "patient/Observation.cud"),
      "covered"
    ),
  ...
  expect_error(
    coverage(rep("x", 257L), paste0("x", seq_len(257))),
    "size limit"
  )
})
...
test_that("SMART v1 aliases require explicit policy and keep wire spellings", {
  ...
  expect_setequal(
    strsplit(utils::URLdecode(parse_query_param(url, "scope")), " ")[[1L]],
    requested
  )
  expect_error(
...
test_that("granular scope comparisons never guess query implication", {
  ...
  expect_identical(
    smart_scope_coverage(scope, same_parts)[["status"]],
    "covered"
  )
  expect_identical(
    smart_scope_coverage(scope, "patient/*.rs")[["status"]],
    "covered"
  )
  expect_identical(
  ...
  for (query in c(
    ...
    "code=x&_filter=y",
    "_id:exact=x",
    "_has:Observation:patient:code=x",
    "code=%zz",
    "code=",
    ""
  )) {
    ...
    expect_identical(
      smart_scope_coverage(unknown, unknown)[["status"]],
      "indeterminate"
    )
  }
  expect_identical(
    smart_scope_coverage(
      "patient/Observation.rs?a=1&b=2",
      "patient/Observation.rs?b=2&a=1"
    )[["status"]],
    "indeterminate"
  )
})
...
test_that("granular scopes accept simple common FHIR search parameters", {
  for (query in c(
    ...
    "_tag=http://example.org|reviewed",
    "_profile=http://example.org/Patient",
    "_security=http://example.org|R",
    "code=example&_id=example"
  )) {
    scope <- paste0("user/Patient.rs?", query)
    expect_identical(smart_scope_coverage(scope, scope)[["status"]], "covered")
    expect_identical(
      smart_scope_coverage(scope, "user/Patient.rs")[["status"]],
      "covered"
    )
    expect_identical(
      smart_scope_coverage("user/Patient.rs", scope)[["status"]],
      "insufficient"
    )
    different <- paste0(scope, "-other")
    expect_identical(
      smart_scope_coverage(scope, different)[["status"]],
      "indeterminate"
    )
    client <- smart_client(
...
test_that("SMART response scope is explicit, including the empty grant", {
  ...
  for (bad in list(
    ...
    expect_error(
      verify_token_set(client, c(base, list(scope = bad)), nonce = NULL),
...
test_that("SMART parsing accepts empty scope only with explicit opt-in", {
  for (kind in c(
    ...
    expect_identical(
      parse_token_response(response, allow_empty_scope = TRUE)[["scope"]],
      ""
    )
  }
  for (value in c("null", "[]", "[\"x\"]", "false", "42")) {
    ...
    expect_error(
      parse_token_response_json(body, allow_empty_scope = TRUE),
      class = "shinyOAuth_parse_error"
    )
  }
  expect_error(
    parse_token_response_json(
      '{"scope":"","scope":"x"}',
...
test_that("SMART required permissions and refresh continuity use semantics", {
  ...
  expect_no_warning(verify_token_set(
    client,
    smaller,
    nonce = NULL,
    is_refresh = TRUE,
    prior_granted_scopes = initial[["granted_scopes"]]
  ))
  for (scope in c(
    "patient/Patient.cruds",
    "patient/*.rs",
    "patient/Patient.r custom"
  )) {
    expect_error(
      verify_token_set(
        client,
        c(base, list(scope = scope)),
        nonce = NULL,
        is_refresh = TRUE,
        prior_granted_scopes = initial[["granted_scopes"]]
      ),
  ...
  expect_error(
    verify_token_set(
      client,
      c(base, list(scope = "patient/Observation.rs")),
      nonce = NULL
    ),
    "required permissions"
  )
...
test_that("SMART scope evidence survives the actual callback and refresh paths", {
  ...
  local_mocked_bindings(
    req_with_retry = function(req, ...) {
      ...
      if (!is.null(response_scope)) {
        fields[["scope"]] <- response_scope
      }
      httr2::response(
        url = req[["url"]],
        status = 200,
  ...
  expect_setequal(
    ...
    c("patient/Patient.r", "patient/Patient.s")
  )
...
test_that("SMART introspection cannot supply missing evidence or widen a grant", {
  client <- smart_scope_test_client(
    scopes = c("patient/Patient.rs", "patient/Observation.rs")
  )
