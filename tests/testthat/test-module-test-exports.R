  ...
  shiny::testServer(oauth_module_server,
    args = list(id = "auth", client = client, auto_redirect = FALSE), {
      values$token <- OAuthToken(
        ...
        id_token = "secret-id", expires_at = as.numeric(Sys.time()) + 3600,
        userinfo = list(email = "secret-email")
      )
      values$browser_token <- "secret-browser"
      values$pending_callback <- list(code = "secret-code", payload = "secret-state",
      ...
      expect_true(all(vapply(snapshot, function(x) {
