...
test_that("optional JWT claims distinguish omission, null, scalars and arrays", {
  ...
  encode <- function(claims) {
    paste(
      ...
      base64url_encode(charToRaw("signature")),
      sep = "."
    )
  }
  surfaces <- list(
    id_token = list(
      validate = function(jwt) {
        validate_id_token(client, jwt, expected_access_token = "access")
      },
      values = list(
        nbf = now - 1,
        azp = client@client_id,
        at_hash = compute_at_hash("access", "RS256")
      ),
      error = "shinyOAuth_id_token_error"
    ),
    jarm = list(
      validate = function(jwt) {
        validate_jarm_claims(client, parse_jwt_payload(jwt))
      },
  ...
  for (surface in surfaces) {
    for (claim in names(surface[["values"]])) {
      ...
      for (invalid in list(
        NULL,
        list(surface[["values"]][[claim]]),
        list(),
        TRUE
      )) {
        claims[claim] <- list(invalid)
        expect_error(
          surface[["validate"]](encode(claims)),
          class = surface[["error"]]
        )
      }
    }
