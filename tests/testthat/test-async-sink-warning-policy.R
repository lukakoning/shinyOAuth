test_that("captured sink warnings cannot reject successful async results", {
  ...
  value <- withCallingHandlers({
    shinyOAuth:::warn_event_sink_failure("Sample sink failed", "sample detail")
    "successful result"
  }, warning = function(w) {
  ...
  wrapped <- list(.shinyOAuth_async_wrapped = TRUE, value = value,
                  warnings = warnings, messages = list())
  # Replay outside testthat's warning handlers so the strict policy really runs.
  replay <- function(wrapped) callr::r(function(wrapped, path) {
    pkgload::load_all(path, quiet = TRUE, helpers = FALSE)
    options(warn = 2)
    list(
      value = tryCatch(shinyOAuth:::replay_async_conditions(wrapped), error = identity),
      warn = getOption("warn")
    )
