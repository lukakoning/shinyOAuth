for (base in c("/", "/app/")) for (method in c("GET", "POST")) for (async in c(FALSE, TRUE)) {
  test_that(paste("managed Request Objects use the application base", base, method, async), {
    ...
    manager <- oauth_connections(list(site = client), "https://app.example")
    ui <- oauth_connections_ui(shiny::fluidPage("App"), "health", manager, app_base_path = base)
    sent <- NULL
