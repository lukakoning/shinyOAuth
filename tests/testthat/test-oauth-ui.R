test_that("OAuth UI protects HTML and preserves custom response semantics", {
  req <- list(REQUEST_METHOD = "GET", PATH_INFO = "/", QUERY_STRING = "code=synthetic")
  ...
  custom <- shiny::httpResponse(201L, content = "<html>custom</html>",
...
test_that("callback asset requests send no Referer in a real browser", {
  ...
  dep <- htmltools::htmlDependency("referrer-probe", "1.0",
    src = c(href = "/assets"), script = "probe.js")
  req <- list(REQUEST_METHOD = "GET", PATH_INFO = "/", QUERY_STRING = "code=SYNTHETIC&state=SYNTHETIC")
  ...
  app$get("/", function(req, res) {
    res$set_header("Referrer-Policy", app$locals$policy)$set_type("text/html")$send(app$locals$html)
  })
  app$get("/assets/probe.js", function(req, res) {
    referrer <- req$get_header("referer")
    if (is.null(referrer)) referrer <- ""
    res$set_type("application/javascript")$send(paste0(
      "window.probeReferrer = ", jsonlite::toJSON(referrer, auto_unbox = TRUE), ";"))
  })
