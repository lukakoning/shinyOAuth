...
testthat::test_that("Keycloak runner bounds pull retries and preserves later failures", {
  ...
  fixture <- normalizePath(testthat::test_path("..", "keycloak-runner-fixture.sh"), winslash = "/")
  for (scenario in list(
    ...
    writeLines(c("#!/usr/bin/env bash", "exit 0"),
      file.path(script_dir, "prepare-tls.sh"), useBytes = TRUE)
    log <- file.path(root, "commands.log")
    result <- processx::run(bash, c(fixture, runner, log, as.character(scenario)),
      error_on_status = FALSE, timeout = 10000)
    exhausted <- scenario[["failures"]] == 3L
    expected_status <- if (exhausted) 42L else if (scenario[["start"]] != 0L) {
      scenario[["start"]]
    } else scenario[["test"]]
    testthat::expect_identical(result[["status"]], expected_status, info = result[["stderr"]])
    attempts <- min(3L, scenario[["failures"]] + 1L)
    expected <- "docker compose pull --policy missing keycloak"
    if (attempts > 1L) for (attempt in seq_len(attempts - 1L)) {
      expected <- c(expected, paste("sleep", attempt * 5L),
        "docker compose pull --policy missing keycloak")
    }
    if (!exhausted) {
      expected <- c(expected, "docker compose up -d --pull never")
      if (scenario[["start"]] == 0L) expected <- c(expected, "Rscript integration/keycloak/run-tests.R")
    }
    if (expected_status != 0L) {
      expected <- c(expected, "docker compose logs --no-color")
      testthat::expect_identical(readLines(file.path(script_dir, ".generated", "keycloak-compose.log")),
        "synthetic Docker logs")
    }
    testthat::expect_identical(readLines(log), c(expected, "docker compose down -v"))
  }
})
