...
test_that("ORCID selects the environment and retains confidential OIDC checks", {
  for (sandbox in c(FALSE, TRUE)) {
    ...
    local_research_discovery(doc, paste0(issuer, "/.well-known/openid-configuration"))
    ...
    client <- oauth_client(provider, "client", "secret",
      redirect_uri = "https://app.example/callback", scopes = "openid"
    )
  ...
  for (value in list(NA, NULL, "true", 1, c(TRUE, FALSE))) {
    expect_error(oauth_provider_orcid(sandbox = value),
...
test_that("Cognito discovers login-domain endpoints while retaining the pool issuer", {
  ...
  for (issuer in issuers) {
    doc <- research_discovery(issuer, endpoint_base = "https://login.example.com/oauth2")
    ...
    expect_identical(provider@issuer, issuer)
    expect_identical(provider@auth_url, "https://login.example.com/oauth2/authorize")
    expect_identical(provider@token_url, "https://login.example.com/oauth2/token")
...
test_that("authentik keeps its application issuer slash and rejects global mode", {
  ...
  expect_error(
    ...
    "issuer mismatch", class = "shinyOAuth_config_error"
  )
  for (slug in c("", "../other", "a/b", "x?y", "x#y", "x%2fy")) {
    expect_error(oauth_provider_authentik("https://auth.example.com", slug),
      class = "shinyOAuth_input_error"
    )
  }
})
...
test_that("new discovery presets preserve issuer and signing-key host validation", {
  ...
  for (value in list(NULL, NA_character_, "", c("https://a", "https://b"))) {
    ...
    expect_error(oauth_provider_authentik(value, "app"), class = "shinyOAuth_input_error")
    expect_error(oauth_provider_cognito(value))
  }
})

test_that("Globus configures RS512 without weakening generic discovery", {
  local_mocked_bindings(req_with_retry = function(...) stop("unexpected network"))
  ...
  local_research_discovery(doc, paste0(provider@issuer, "/.well-known/openid-configuration"))
...
test_that("Globus login validates an RS512 signature, nonce, and UserInfo subject", {
  ...
  client <- oauth_client(provider,
    client_id = "globus-test-client", client_secret = "test-secret",
  ...
  local_mocked_bindings(
    swap_code_for_token_set = function(...) {
      list(access_token = "access", token_type = "Bearer", id_token = jwt,
    ...
    req_with_retry = function(req, ...) {
      ...
      httr2::response(
        ...
        body = charToRaw(as.character(jsonlite::toJSON(
          list(sub = subject), auto_unbox = TRUE
        )))
      )
    }
  )
  login <- function(signing_key = key, wrong_nonce = FALSE) {
    ...
    claims <- list(
      iss = provider@issuer, aud = client@client_id, sub = "researcher",
      iat = now, exp = now + 120,
      nonce = if (wrong_nonce) "wrong" else parse_query_param(url, "nonce", decode = TRUE)
    )
    encode <- function(value) {
      base64url_encode(charToRaw(as.character(jsonlite::toJSON(value, auto_unbox = TRUE))))
    }
    payload <- paste(encode(header), encode(claims), sep = ".")
    signature <- openssl::signature_create(charToRaw(payload),
      hash = openssl::sha512, key = signing_key
    )
    jwt <<- paste(payload, base64url_encode(signature), sep = ".")
    handle_callback(client, code = "code",
  ...
  expect_error(login(openssl::rsa_keygen(2048)), class = "shinyOAuth_id_token_error")
