test_that("raw provider defaults preserve issuer-derived security policies", {
  ...
  for (issuer in c(NA_character_, "https://example.test")) {
    ...
    for (field in c("use_nonce", "id_token_required", "id_token_validation",
...
test_that("raw clients with a DPoP key require DPoP access tokens by default", {
  ...
  args <- list(provider = provider, client_id = "test", client_secret = "",
  ...
  expect_error(verify_token_type_allowlist(do.call(OAuthClient, args),
    list(access_token = "opaque", token_type = "Bearer")), class = "shinyOAuth_token_error")
  args[["dpop_require_access_token"]] <- FALSE
  expect_false(do.call(OAuthClient, args)@dpop_require_access_token)
})

test_that("raw OIDC clients reject token sets without an ID token", {
  provider <- OAuthProvider(name = "test", issuer = "https://example.test",
    auth_url = "https://example.test/auth", token_url = "https://example.test/token")
  client <- OAuthClient(provider = provider, client_id = "test", client_secret = "test-secret",
    redirect_uri = "http://localhost:8100", scopes = "openid", state_key = strrep("a", 64))
  expect_error(verify_token_set(client,
    list(access_token = "opaque", token_type = "Bearer", expires_in = 3600),
    nonce = "expected"), class = "shinyOAuth_id_token_error")
})
