test_that("HTTP paths are omitted by default and route export is explicit", {
  ...
  expect_identical(shinyOAuth:::otel_http_url_full(url), "https://example.test/")
  req <- list(REQUEST_METHOD = "GET", PATH_INFO = "/users/private-name",
              HTTP_HOST = "example.test", QUERY_STRING = "secret=1")
  ...
  expect_identical(shinyOAuth:::otel_http_url_full(url), "https://example.test/users/:id")
  for (bad in list(function(path) stop("secret"), function(path) "//secret",
                  function(path) "/safe?secret", function(path) NA_character_)) {
...
test_that("HTTP request metadata is bounded and free of control characters", {
  for (redact in c(TRUE, FALSE)) {
    local_options(shinyOAuth.audit_redact_http = redact,
                  shinyOAuth.telemetry_path_scrubber = identity)
    req <- list(REQUEST_METHOD = paste0("GET\r\n", strrep("x", 100)),
