test_that("audit and OTel free-form details require exposure permission", {
  ...
  event <- list(type = "error", message = detail,
  ...
  expect_false(any(grepl("private detail", unlist(otel_event_attributes(event)))))
  ...
  expect_lte(nchar(sanitize_diagnostic_text(strrep("é", 600)), type = "bytes"), 512)
...
test_that("provider HTTP descriptions respect exposure and OAuth text syntax", {
  local_options(shinyOAuth.expose_error_body = FALSE)
  response <- httr2::response(status = 400, headers = list("content-type" = "application/json"),
    body = charToRaw('{"error":"invalid_grant","error_description":"private detail"}'))
  error <- tryCatch(err_http("Token request failed", response), error = identity)
  ...
  for (text in c("bad\r\ntext", "bad\ttext", 'bad"text', "bad\\text", "é")) {
    ...
    expect_error(validate_untrusted_query_param("error_description", text, 4096),
                 "printable ASCII")
  }
...
test_that("sink failures withhold details by default and sanitize opt-in warnings", {
  local_options(shinyOAuth.expose_error_body = FALSE,
                shinyOAuth.audit_hook = function(event) stop("{dsn}\r\nhttps://u:password@example.test/?secret"))
  expect_warning(emit_trace_event(list(type = "error")), "details withheld")
  local_options(shinyOAuth.expose_error_body = TRUE)
  observed <- NULL
  withCallingHandlers(emit_trace_event(list(type = "error")), warning = function(w) {
  ...
  for (expose in c(FALSE, TRUE)) {
    ...
    shiny::testServer(oauth_module_server,
      args = list(id = "auth", client = make_test_client(), auto_redirect = FALSE), {
        ...
        values[[".process_query"]](paste0("?error=access_denied&error_description=Private%20detail&state=", state))
        session$flushReact()
        expect_identical(values[["error"]], "access_denied")
        if (expose) expect_identical(values[["error_description"]], "Private detail")
        else expect_null(values[["error_description"]])
      })
  }
})

test_that("direct OTel log bodies apply diagnostic exposure policy", {
  local_options(shinyOAuth.otel_logging_enabled = TRUE,
                shinyOAuth.expose_error_body = FALSE)
  seen <- NULL
  local_mocked_bindings(log = function(msg, ...) seen <<- msg, .package = "otel")
