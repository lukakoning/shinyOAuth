smart_context_fixture <- function(
  ...
  client <- smart_client(
    ...
    scopes = c("launch/patient", scopes),
...
test_that("SMART context requires actual patient evidence and validates scalar types", {
  ...
  expect_identical(
    record[["token"]]@smart_context[["patient"]],
    "example-patient"
  )
...
test_that("refresh carries omitted context but distinguishes clear and change", {
  ...
  expect_error(
    smart_update_token_context(record[["client"]], token, previous),
    "patient context"
  )
...
test_that("SMART helpers restrict requests and redact general summaries", {
  ...
  expect_false(any(grepl(
    "example-patient|example-encounter",
    unlist(ref[["summary"]]())
  )))
  called <- FALSE
  local_mocked_bindings(
    perform_resource_req = function(token, url, ...) {
      called <<- TRUE
      url[["url"]]
    },
    .package = "shinyOAuth"
  ...
  record[["token"]]@smart_context[[
    "fhirUser"
  ]] <- "https://other.example/Practitioner/example"
  record[["token"]]@id_token <- jose::jwt_encode_sig(
    jose::jwt_claim(sub = "example-user"),
    openssl::rsa_keygen(2048)
  )
  record[["token"]]@id_token_validated <- TRUE
  foreign <- OAuthConnection[["new"]](
    "another-reference",
    record[["client"]],
    function() record
  )
  ...
  record[["token"]]@smart_context[[
    "fhirUser"
  ]] <- "https://ehr.example/fhir/R4/Practitioner/example"
  expect_identical(
    smart_fhir_user(foreign),
...
test_that("SMART read helpers negotiate FHIR JSON while generic requests keep their defaults", {
  ...
  record[["token"]] <- smart_update_token_context(
    record[["client"]],
    record[["token"]]
  )
  ref <- OAuthConnection[["new"]](
  ...
  expect_identical(
    httr2::resp_body_json(user)[["resourceType"]],
    "Practitioner"
  )
...
test_that("retained credentials preserve interpreted context without plaintext", {
  ...
  opened <- connection_credentials_open(
    sealed,
    owner,
    id,
    record[["client"]],
...
test_that("Patient identity reads accept only the matching patient context", {
  ...
  local_mocked_bindings(
    perform_resource_req = function(token, url, follow_redirect, ...) {
  ...
  for (reference in c(
    ...
    expect_identical(
      smart_fhir_user(ref),
      resolve_bound_resource(record[["client"]]@smart[["fhir_base"]], reference)
    )
  }
...
test_that("identity scopes authorize the signed same-base fhirUser reference", {
  ...
  record <- smart_identity_fixture(
    "https://ehr.example/fhir/R4/identity/example"
  )
  ref <- OAuthConnection[["new"]](
    "signed-identity",
    record[["client"]],
