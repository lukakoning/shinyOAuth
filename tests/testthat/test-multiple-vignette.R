test_that("the first three multiple-authorization chunks assemble a complete app", {
  ...
  skip_if_not(file.exists(path), "Vignette source is unavailable in installed tests")
  lines <- readLines(path, warn = FALSE)
  starts <- which(grepl("^```\\{r ", lines))
  expect_identical(sub("^```\\{r ([^,]+),.*$", "\\1", lines[starts[1:3]]),
  ...
  withr::local_envvar(c(SERVICE_A_CLIENT_ID = "synthetic-a", SERVICE_A_CLIENT_SECRET = "synthetic-a-secret",
    SERVICE_B_CLIENT_ID = "synthetic-b", SERVICE_B_CLIENT_SECRET = "synthetic-b-secret"))
  ...
  env[["runApp"]] <- function(appDir, ...) appDir
  app <- NULL
  for (chunk in chunks) app <- eval(chunk, envir = env)
