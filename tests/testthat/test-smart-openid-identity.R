test_that("SMART openid identity is explicit and does not require fhirUser", {
  ...
  client <- smart_client(site, "example", "https://app.example/callback",
    scopes = character(), identity = "openid")
  ...
  query <- httr2::url_parse(prepare_call(client, valid_browser_token()))$query
  ...
  expect_error(client@provider@id_token_validation <- FALSE, "identity validation|id_token_validation")
  expect_error(client@scopes <- c("openid", "fhirUser"), "Identity scopes require")
  expect_error(smart_client(site, "example", "https://app.example/callback",
    scopes = "fhirUser", identity = "openid"), "Identity scopes require")
  expect_error(smart_client(smart_client_fixture(), "example", "https://app.example/callback",
    scopes = character(), identity = "openid"), "sso-openid-connect")
  none <- smart_client(site, "example", "https://app.example/callback", scopes = "user/Patient.r")
...
test_that("openid-only SMART callbacks and refresh expose a validated OIDC subject", {
  ...
  client <- smart_client(smart_client_fixture(oidc = TRUE), "example",
    "https://app.example/callback", scopes = character(), identity = "openid")
  ...
  response <- list(access_token = "example-access", refresh_token = "example-refresh",
    token_type = "Bearer", expires_in = 300, scope = "openid",
    id_token = jose::jwt_encode_sig(do.call(jose::jwt_claim, claims), fixture$key))
  token <- handle_callback(client, code = "example-code", payload = parse_query_param(url, "state"),
  ...
  response$id_token <- jose::jwt_encode_sig(do.call(jose::jwt_claim, claims), fixture$key)
  ...
  shiny::testServer(oauth_connections_server, session = manager_test_session(),
    args = list(id = "health", manager = manager), {
      ...
      expect_identical(connection$identity()$id_token_claims,
...
test_that("openid-only SMART keeps normal ID-token validation requirements", {
  ...
  for (invalid in c("missing", "nonce", "iss", "aud", "signature")) {
    claims <- fixture$claims
    claims$fhirUser <- NULL
    if (invalid %in% c("nonce", "iss", "aud")) claims[[invalid]] <- "unexpected"
    key <- if (invalid == "signature") openssl::rsa_keygen(2048) else fixture$key
    response <- list(access_token = "example-access", token_type = "Bearer",
      expires_in = 300, scope = "openid")
    if (invalid != "missing") {
      response$id_token <- jose::jwt_encode_sig(do.call(jose::jwt_claim, claims), key)
    }
    expect_error(verify_token_set(client, response, nonce = "expected-nonce"),
