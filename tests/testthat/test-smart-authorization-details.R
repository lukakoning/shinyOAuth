...
test_that("initial SMART context and scopes use only the configured location", {
  client <- smart_client(
    ...
    scopes = c("user/Patient.rs", "user/Observation.r"),
  ...
  body <- list(
    ...
    token_type = "Bearer",
    expires_in = 300,
    scope = "user/Patient.rs user/Observation.r",
    patient = "top-patient",
    encounter = "top-encounter",
    authorization_details = list(
      smart_details_entry(
        "https://other.example/fhir",
  ...
  expect_identical(
    token@extra_fields[["authorization_details"]],
    body[["authorization_details"]]
  )
  record <- list(client = client, token = token)
  ref <- OAuthConnection[["new"]]("location-reference", client, function() {
    record
  })
  local_mocked_bindings(
    perform_resource_req = function(token, url, ...) url[["url"]],
    .package = "shinyOAuth"
  )
  expect_identical(
    smart_patient(ref),
    paste0(client@smart[["fhir_base"]], "/Patient/local-patient")
  )
  expect_error(
    ref[["request"]](
      "fhir",
      "Observation/example",
      required_scopes = "user/Observation.r"
    ),
  ...
  expect_error(
    ref[["request"]](
      "fhir",
      "https://other.example/fhir/Patient/other-patient"
    ),
    "approved base"
  )
  ...
  expect_setequal(
    fallback@granted_scopes,
    c("user/Patient.rs", "user/Observation.r")
  )
  ...
  expect_identical(
    smart_details_callback(client)@smart_context[["patient"]],
    "top-patient"
  )
  body[["authorization_details"]] <- list(smart_details_entry(
    paste0(client@smart[["fhir_base"]], "/"),
    patient = "local-patient",
...
test_that("refresh applies location overrides before scope and context acceptance", {
  ...
  narrowed <- refresh_token_dispatch(
    client,
    initial,
    scope_request = refresh_scope_request(client, initial, "user/Patient.r")
  )
...
test_that("invalid or ambiguous local authorization details are rejected", {
  ...
  for (details in list(
    ...
    "unsupported",
    entry,
    list(list(type = "smart_on_fhir")),
    list(entry, entry),
    list(smart_details_entry(paste0(client@smart[["fhir_base"]], "/../R4")))
  )) {
    expect_error(
      verify_token_set(
        client,
        c(base, list(authorization_details = details)),
        NULL
      ),
  ...
  for (scope in list(NULL, 42, "", "user/Patient.r")) {
    ...
    expect_error(
      verify_token_set(
        client,
        c(base, list(authorization_details = list(entry))),
        NULL
      ),
      "scope|permissions"
    )
  ...
  token <- OAuthToken(
    ...
    extra_fields = list(
      patient = "top-patient",
      authorization_details = list(entry)
    )
  )
  ...
  for (field in c("type", "locations", "fhirVersions")) {
    ...
    expect_error(
      verify_token_set(
        client,
        c(base, list(authorization_details = list(malformed))),
        NULL
      ),
      "authorization_details"
    )
