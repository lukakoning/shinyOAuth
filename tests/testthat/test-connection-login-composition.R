...
for (post in c(FALSE, TRUE)) test_that(paste("one wrapper bridges ordinary OIDC and SMART", post), {
  ...
  login <- oauth_client(oauth_provider(name = "Login", issuer = "https://login.example",
    auth_url = "https://login.example/authorize", token_url = "https://login.example/token",
    token_auth_style = "public"), "login", redirect_uri = redirects[[1L]], scopes = "openid",
    response_mode = mode, authorization_server_mode = "multi_redirect_uri",
    authorization_server_redirect_uris = redirects)
  fhir <- smart_client(smart_client_fixture(), "fhir", redirects[[2L]], scopes = "user/Patient.r",
    response_mode = mode, authorization_server_mode = "multi_redirect_uri",
    authorization_server_redirect_uris = redirects)
  ...
  shiny::withReactiveDomain(session, shiny::isolate({
    ...
    expect_false(client_uses_smart(login))
    expect_error(oauth_connections_ui(shiny::fluidPage(), "health", manager,
      additional_clients = list(login = fhir)), "distinct from managed")
  }))
})

for (post in c(FALSE, TRUE)) test_that(paste("ordinary continuations survive a retired browser owner", post), {
  redirects <- c("https://app.example/login/callback", "https://app.example/fhir/callback")
  mode <- if (post) "form_post" else "query"
  login <- oauth_client(oauth_provider(name = "Login", issuer = "https://login.example",
    issuer_thus_oidc = FALSE, auth_url = "https://login.example/authorize",
    token_url = "https://login.example/token", token_auth_style = "public"), "login",
    redirect_uri = redirects[[1L]], response_mode = mode,
    authorization_server_mode = "multi_redirect_uri", authorization_server_redirect_uris = redirects)
  fhir <- smart_client(smart_client_fixture(), "fhir", redirects[[2L]], scopes = "user/Patient.r",
    response_mode = mode, authorization_server_mode = "multi_redirect_uri",
    authorization_server_redirect_uris = redirects)
  manager <- oauth_connections(list(fhir = fhir), "https://app.example", retention = "browser",
