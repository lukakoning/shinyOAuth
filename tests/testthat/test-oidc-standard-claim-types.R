test_that("ID tokens and both UserInfo formats preserve standard claim types", {
  client <- oauth_client(
    oauth_provider(
      name = "claims", issuer = "https://example.test",
      auth_url = "https://example.test/auth", token_url = "https://example.test/token",
      userinfo_url = "https://example.test/userinfo", use_nonce = FALSE,
  ...
  base <- as.character(jsonlite::toJSON(list(
  ...
  sign_json <- function(json) {
    input <- paste(
      ...
      base64url_encode(charToRaw(json)), sep = "."
    )
    signature <- openssl::signature_create(charToRaw(input), openssl::sha256, key)
    paste(input, base64url_encode(signature), sep = ".")
  }
  check <- function(json, channel) {
    jwt <- sign_json(json)
    if (channel == "id_token") return(validate_id_token(client, jwt))
    signed <- channel == "userinfo_jwt"
    response <<- httr2::response(
      status = 200, headers = list(
  ...
  invalid <- list(
    ...
    amr = c('"pwd"', 'true', 'null', '{}', '{"method":"pwd"}',
      '["pwd",1]', '[["pwd"]]', '[null]')
  )
  valid <- c(
    '',
  ...
  for (channel in c("id_token", "userinfo_json", "userinfo_jwt")) {
    ...
    for (field in names(invalid)) {
      for (value in invalid[[field]]) {
        json <- paste0(substr(base, 1L, nchar(base) - 1L),
          ',"', field, '":', value, '}')
        expect_error(check(json, channel), field,
          class = if (channel == "id_token") "shinyOAuth_id_token_error" else "shinyOAuth_userinfo_error"
        )
      }
    }
  }
