test_that("accepted leaf-first certificate layouts complete an mTLS exchange", {
  ...
  server <- processx::process$new(python, mtls_pem_fixture("roundtrip-server.py"),
                                 stdout = "|", stderr = "|")
  ...
  client <- oauth_client(
    provider = client@provider, client_id = "test",
  ...
  for (layout in c(cert, bundle)) {
    ...
    expect_identical(httr2::resp_body_string(httr2::req_perform(req)),
                     "client certificate accepted")
  }
  writeLines(c(readLines(ca), readLines(cert)), bundle)
  expect_error(req_apply_mtls_client_certificate(req, client),
               "must put the client certificate.*first")
  expect_error(oauth_client(
    provider = client@provider, client_id = "test", redirect_uri = "http://localhost:8100",
    mtls_client_cert_file = bundle, mtls_client_key_file = client@mtls_client_key_file
  ), "must put the client certificate.*first")
})
