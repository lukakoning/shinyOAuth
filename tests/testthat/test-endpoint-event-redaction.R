test_that("endpoint values are redacted recursively for native and OTel events", {
  ...
  event <- list(type = "test_endpoint", endpoint = url,
  ...
  expect_false(any(grepl("private|password|user@", unlist(otel_event_attributes(event)))))
  seen <- NULL
  local_options(shinyOAuth.audit_hook = function(event) seen <<- event)
