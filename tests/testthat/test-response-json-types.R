test_that("token JSON preserves singleton arrays until scalar validation", {
  for (field in c(
    ...
    body <- paste0(
      ...
      field,
      '":[',
      if (field == "expires_in") "60" else '"value"',
      ']}'
    )
...
test_that("OIDC UserInfo validates JSON scalars and preserves profile arrays", {
...
test_that("malformed wire scopes never become verified grants", {
  ...
  for (wire in c(
    ...
    intro <- list(
      ...
      raw = jsonlite::fromJSON(
        paste0('{"active":true,"scope":', wire, '}'),
        simplifyVector = FALSE
      )
    )
    token <- OAuthToken(access_token = "synthetic", token_type = "Bearer")
    expect_error(
      shinyOAuth:::enforce_token_introspection_policy(cli, token, intro),
      "scope"
    )
  ...
  expect_identical(
    shinyOAuth:::as_scope_tokens(list("read\twrite")),
    c("read", "write")
  )
})
