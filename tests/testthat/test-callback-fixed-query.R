...
test_that("SMART callbacks retain registered queries and HTTPS policy", {
  ...
  create <- function(uri, ...) {
    smart_client(site, "example", uri, scopes = "user/Patient.r", ...)
  }
  for (mode in c("query", "form_post")) {
    ...
    expect_identical(
      oauth_callback_route_matches(
        paste0(registered, "&code=example&state=example"),
        client@redirect_uri
      ),
      TRUE
    )
    expect_identical(
      oauth_callback_route_matches(
        sub("tenant=one", "tenant=two", registered, fixed = TRUE),
        client@redirect_uri
      ),
      FALSE
    )
  ...
  for (uri in c(
    "http://app.example/callback?tenant=one",
    "http://localhost/callback?tenant=one",
    paste0(registered, "#fragment"),
    "https://user@app.example/callback?tenant=one"
  )) {
    expect_error(create(uri), "SMART redirect_uri")
  }
  expect_error(
    create("https://app.example/callback?%73tate=fixed"),
    "callback-reserved"
  )
