...
test_that("authorization POST preserves fields, fixed query and browser transaction", {
  ...
  client <- make_test_client(
    use_nonce = TRUE,
    scopes = c("openid", paste0("permission:", seq_len(40))),
  ...
  client@provider@auth_url <- paste0(
    ...
    "?fixed=a%2Bb&client_id=abc"
  )
  client@provider@extra_auth_params <- list(
    login_hint = 'A+B & <literal> "quote"',
    submit = "extension"
  )
  ...
  expect_identical(
    unlist(fields[names(fields) == "resource"], use.names = FALSE),
    client@resource
  )
  expect_identical(
    fields[["login_hint"]],
    client@provider@extra_auth_params[["login_hint"]]
  )
  ...
  expect_error(
    state_payload_decrypt_validate(changed, fields[["state"]]),
    "client policy"
  )
  worker <- prepare_client_for_worker(client)
  expect_identical(worker@authorization_method, "POST")
  expect_identical(
    state_client_policy_fingerprint(worker),
...
test_that("GET remains a URL and URL-only helpers reject POST before storing state", {
  ...
  for (method in c("post", "PUT", "")) {
    expect_error(client@authorization_method <- method, "GET or POST")
  }
})
...
test_that("POST keeps PAR requirements and expiry metadata", {
  ...
  expect_identical(attr(request, "shinyOAuth.par_expires_in", exact = TRUE), 60)
  expect_gt(
    attr(request, "shinyOAuth.par_expires_at", exact = TRUE),
    Sys.time()
  )
  ...
  before <- client@state_store[["keys"]]()
  expect_error(
    prepare_authorization_request(client, valid_browser_token()),
    "PAR refused"
  )
  expect_setequal(client@state_store[["keys"]](), before)
})
...
test_that("POST preserves signed Request Object and published reference composition", {
  ...
  published <- NULL
  request <- prepare_authorization_request(
    client,
    valid_browser_token(),
  ...
  expect_identical(
    post_fields(request)[["request_uri"]],
    "https://app.example/request/fixture"
  )
...
test_that("POST fails cleanly on conflicting fixed fields and form limits", {
  ...
  client@authorization_method <- "POST"
  for (extra in list(
    list(login_hint = "line\nbreak"),
    list(`_charset_` = "provider-value"),
    list(`_CHARSET_` = "provider-value"),
    list(`_cHaRsEt_` = "provider-value"),
    list(login_hint = strrep("x", 131073)),
    list(login_hint = strrep("~", 44000)),
...
test_that("POST browser handler preserves literal fields and validates form messages", {
  ...
  output <- system2(
    Sys.which("node"),
    c(
      shQuote(test_path("..", "browser-authorization-post.cjs")),
...
test_that("prepared POST work requires live state and an unexpired PAR reference", {
  ...
  expect_identical(
    finish_prepared_authorization(result, client, prepared),
    result
  )
  ...
  for (action in c("success", "logout", "failure", "replacement")) {
    ...
    shiny::testServer(
      ...
      ),
      {
        ...
        expect_identical(
          work[["args"]][["worker"]]@authorization_method,
          "POST"
        )
        ...
        if (action == "failure") {
          ...
          work[["resolve"]](list(
            method = "POST",
            url = client@provider@auth_url,
            fields = list(
              list(name = "client_id", value = client@client_id),
        ...
        if (action == "success") {
          expect_identical(sent[[1L]][["method"]], "POST")
          expect_identical(
            post_fields(sent[[1L]])[["request_uri"]],
            "urn:fixture:par"
          )
        } else {
          expect_length(sent, 0L)
