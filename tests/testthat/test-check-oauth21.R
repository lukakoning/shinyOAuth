...
test_that("JWT audience is mandatory while legacy typing remains an advisory", {
  ...
  expect_identical(
    parse_jwt_payload(assertion)[["aud"]],
...
test_that("endpoint scope and effective overrides agree with actual requests", {
  ...
  expect_identical(
    ...
    client@provider@revocation_url
  )
  ...
  expect_true(report[["configuration_compliant"]])
  expect_identical(
    oauth21_endpoint_settings(client, client@provider, "introspection")[[
      "style"
    ]],
...
test_that("active relaxations and capacity findings have appropriate applicability", {
  ...
  expect_false(oauth21_finding(report, "callback.capacity")[[
    "affects_verdict"
  ]])
...
test_that("assessment recommendations preserve valid legacy configurations and identify owners", {
  ...
  client@redirect_uri <- "http://127.0.0.1:8100/callback"
  expect_identical(
    oauth21_finding(check_oauth21(client), "redirect.loopback_literal")[[
      "status"
    ]],
    "pass"
  )
  ...
  expect_identical(
    oauth21_finding(report, "tokens.application_policy")[[
      "requirement_source"
    ]],
    "application_policy"
  )
...
test_that("JARM and validated OIDC policies are assessed when selected", {
  ...
  expect_identical(
    oauth21_finding(check_oauth21(client), "parameters.authorization")[[
      "status"
    ]],
    "fail"
  )
...
test_that("assessment canonicalizes local HMAC algorithm names for every endpoint", {
  withr::local_options(list(shinyOAuth.tls_min_version = "1.2"))
  client <- oauth21_test_client(
    list(
      issuer = "https://issuer.example",
...
test_that("assessment checks private-key endpoint overrides without signing", {
  withr::local_options(list(shinyOAuth.tls_min_version = "1.2"))
...
test_that("assessment and PEM mTLS construction agree on the active backend", {
  withr::local_options(list(shinyOAuth.tls_min_version = "1.2"))
  client <- oauth21_test_client(
    list(token_auth_style = "tls_client_auth"),
    mtls_client_cert_file = mtls_pem_fixture("client-cert.pem"),
    mtls_client_key_file = mtls_pem_fixture("client-key.pem")
  )
