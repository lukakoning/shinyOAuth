test_that("concurrent exported refreshes share one dispatch and rotated result", {
  ...
  expect_error(
    refresh_token(client, token, async = TRUE, introspect = TRUE),
  ...
  finish(rotated)
  deadline <- Sys.time() + 3
  while (length(values) < 2L && Sys.time() < deadline) {
    later::run_now(0.01)
  }
...
test_that("failed refreshes release locks and separate clients do not collide", {
  ...
  for (reject in rejectors) {
    reject(simpleError("refresh failed"))
  }
  deadline <- Sys.time() + 3
  while (length(errors) < 2L && Sys.time() < deadline) {
    later::run_now(0.01)
  }
